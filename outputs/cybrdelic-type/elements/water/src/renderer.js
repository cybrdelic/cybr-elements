/* CYBR FLIP III.1 — Three.js dielectric raster renderer.
...
const WATER_FRAGMENT_LEGACY_III=`uniform sampler2D tScene,tBack,tDepth,tFloor;uniform vec2 uResolution;uniform mat4 uViewProjection;
...
// Opaque-scene ray march with physical floor intersection fallback. This is a
// screen-space preview (not an unbiased path tracer); offscreen rays use the
// studio environment rather than stretched edge pixels.
vec3 opaqueRadiance(vec3 origin,vec3 direction,out float confidence){
 confidence=0.;vec3 fallback=environment(direction);float maxT=12.*uScale;
 float ft=direction.y<-.0001?(uFloor-origin.y)/direction.y:-1.;
 if(ft>0.&&ft<maxT){
  vec3 point=origin+direction*ft;
  if(all(greaterThan(point.xz,vec2(.04)*uScale))&&all(lessThan(point.xz,uExtent.xz-vec2(.04)*uScale))){
   vec2 tile=(point.xz/uExtent.xz)*vec2(2.4,1.6);
   fallback=texture2D(tFloor,tile).rgb*.88;maxT=ft+.02*uScale;
   vec2 uv=project(point);float z=-(viewMatrix*vec4(point,1.)).z;
   if(all(greaterThan(uv,vec2(.002)))&&all(lessThan(uv,vec2(.998)))){
    float d=linearDepth(texture2D(tDepth,uv).r);
    if(abs(d-z)<.12*uScale){fallback=texture2D(tScene,uv).rgb;confidence=1.;}
   }
  }
 }
 float previousT=.004*uScale;
 for(int k=1;k<=20;k++){
  float x=float(k)/20.,t=.004*uScale+maxT*x*x;
  vec3 point=origin+direction*t;vec4 clip=uViewProjection*vec4(point,1.);
  if(clip.w<=0.)break;vec2 uv=clip.xy/clip.w*.5+.5;
  if(any(lessThan(uv,vec2(.003)))||any(greaterThan(uv,vec2(.997)))){previousT=t;continue;}
  float z=-(viewMatrix*vec4(point,1.)).z,sd=linearDepth(texture2D(tDepth,uv).r);
  if(z>sd&&z-sd<.09*uScale+maxT/60.){
   float lo=previousT,hi=t;
   for(int j=0;j<4;j++){float middle=.5*(lo+hi);vec3 q=origin+direction*middle;vec2 quv=project(q);float qz=-(viewMatrix*vec4(q,1.)).z;
    if(qz>linearDepth(texture2D(tDepth,clamp(quv,.001,.999)).r))hi=middle;else lo=middle;}
   uv=project(origin+direction*(.5*(lo+hi)));confidence=1.;return texture2D(tScene,clamp(uv,.002,.998)).rgb;
  }
  previousT=t;
 }
 return fallback;
...
void main(){
 ...
 // Six bounded fixed-point corrections locate an exit on the back-depth layer. This
 ...
 float backgroundConfidence;vec3 background=opaqueRadiance(end+outgoing*.003*uScale,outgoing,backgroundConfidence);
 vec3 attenuation=exp(-uAbsorption*path);
 float F=fresnel(clamp(dot(N,V),0.,1.),1.,uIOR);
 float reflectionConfidence;vec3 reflection=opaqueRadiance(vWorld+N*.005*uScale,reflect(-V,N),reflectionConfidence);
...
const WATER_FRAGMENT_BASELINE_II=`uniform sampler2D tScene,tBack,tDepth;uniform vec2 uResolution;uniform mat4 uViewProjection;
uniform float uNear,uFar,uIOR,uScale,uDiagnostic,uFloor;uniform vec3 uAbsorption;uniform vec3 uExtent;
varying float vFoam;varying vec3 vWorld,vNormalW,vView;
${ENVIRONMENT}
float linearDepth(float z){return uNear*uFar/(uFar-z*(uFar-uNear));}
...
void main(){
 ...
 for(int j=0;j<2;j++){
  vec2 q=clamp(endUV,vec2(.001),vec2(.999));vec4 b=texture2D(tBack,q);
  float vd=-(viewMatrix*vec4(end,1.)).z,rayDepth=-(viewMatrix*vec4(transmissionRay,0.)).z;
  if(all(greaterThan(endUV,vec2(.002)))&&all(lessThan(endUV,vec2(.998)))&&b.r>front+.001*uScale&&abs(rayDepth)>.1){
   float correction=clamp((b.r-vd)/rayDepth,-path*.4,path*.4);
   path=clamp(path+correction,.001*uScale,5.5*uScale);end=vWorld+transmissionRay*path;endUV=project(end);
  }
 }
 vec3 exitNormalRaw=texture2D(tBack,clamp(endUV,.002,.998)).gba*2.-1.;
 vec3 exitNormal=dot(exitNormalRaw,exitNormalRaw)>.04?normalize(exitNormalRaw):transmissionRay;
 if(dot(exitNormal,transmissionRay)<0.)exitNormal=-exitNormal;
 vec3 outgoing=refract(transmissionRay,-exitNormal,uIOR);
 bool tir=dot(outgoing,outgoing)<.01;
 if(tir)outgoing=reflect(transmissionRay,exitNormal);
...
void main(){vec4 p=modelViewMatrix*vec4(position,1.);gl_Position=projectionMatrix*p;float diameter=2.*radius*uScale/max(.001,-p.z);
 gl_PointSize=clamp(diameter,1.,48.);vPhase=phase;
 // Preserve projected area instead of brightening every subpixel marker to a
 // fixed minimum-size point. This only changes optical coverage, not physics.
 vOpacity=opacity*min(1.,diameter*diameter/(gl_PointSize*gl_PointSize));vColor=color;vCenter=position;}`;
...
class LiquidRenderer{
 constructor(canvas){
  ...
  this.material=new T.ShaderMaterial({vertexShader:WATER_VERTEX,fragmentShader:WATER_FRAGMENT,
   ...
   tFloor:{value:null},uFloor:{value:.08},uNear:{value:.04},uFar:{value:50},uViewProjection:{value:new T.Matrix4()},uAbsorption:{value:new T.Vector3(.20,.052,.019)},uIOR:{value:1.333},uScale:{value:1},uDiagnostic:{value:0},uExtent:this.extentUniform},side:T.FrontSide});
  ...
  this.opticsMode='raster';this.geometryOptics=null;this.particlePoints.visible=false;this.mode='water';this.shot=0;this.name='breach';this.scale=1;this.orbit=null;this.drag=null;
 ...
 setup(config){
  ...
  const floor=this.box([e[0]-.08*s,.035*s,e[2]-.08*s],[e[0]/2,this.floorY-.018*s,e[2]/2],0xffffff,.03,.48);floor.material.map=tileTexture();this.material.uniforms.tFloor.value=floor.material.map;this.floorMesh=floor;
  ...
  for(const o of config.obstacles){
   ...
   else{const sphereMaterial=this.name==='hero'?new T.MeshPhysicalMaterial({color:0xffffff,transmission:1,ior:1.5,roughness:.035,thickness:2*o.radius}):new T.MeshStandardMaterial({color:0xa1a998,metalness:.8,roughness:.16});mesh=new T.Mesh(new T.SphereGeometry(o.radius,64,40),sphereMaterial);mesh.position.set(...o.center);mesh.castShadow=true;mesh.receiveShadow=true;this.objects.add(mesh);}
 ...
 setRasterProfile(profile='repair'){
  const shaders={'repair':WATER_FRAGMENT,'ii':WATER_FRAGMENT_BASELINE_II,'iii':WATER_FRAGMENT_LEGACY_III};
  if(!shaders[profile])throw Error('Unknown raster profile: '+profile);
  this.material.fragmentShader=shaders[profile];this.material.needsUpdate=true;this.rasterProfile=profile;
  if(['water','thickness'].includes(this.mode))this.water.material=this.material;
 }
 setMode(mode){
 ...
 updateCamera(progress=0){
  ...
  if(this.name==='hero'){pos=this.shot===1?[7.45,3.55,7.7]:[8.0,5.4,8.6];aim=[2.4,1.23,1.6];fov=40;}
 ...
 draw(progress=0){
  if(this.opticsMode==='bvh'&&['water','thickness'].includes(this.mode)&&this.latest?.indices?.length){this.geometryOptics??=new GeometryOptics(this);this.geometryOptics.enable();}
 ...
 info(){const gl=this.renderer.getContext(),ext=gl.getExtension('WEBGL_debug_renderer_info');return {backend:`Three.js r${T.REVISION}`,renderer:ext?gl.getParameter(ext.UNMASKED_RENDERER_WEBGL):gl.getParameter(gl.RENDERER),resolution:[this.width,this.height],vertices:this.geometry.attributes.position.count,triangles:(this.geometry.index?.count??0)/3,programs:this.renderer.info.programs.length,glError:gl.getError(),mode:this.mode,build:'CYBR FLIP III.1',rasterProfile:this.rasterProfile??'repair',optics:this.opticsMode==='bvh'?'BVH dielectric paths / nested media / physical segment absorption':(this.rasterProfile==='iii'?'legacy III screen-space reflection/refraction; raster approximation':'stable 2-step screen-space transmission / environment reflection / Beer-Lambert; raster approximation')};}
}
window.CYBR_WATER_VERTEX=WATER_VERTEX;window.LiquidRenderer=LiquidRenderer;window.CYBR_ENVIRONMENT_GLSL=ENVIRONMENT;window.CYBR_ENV_RADIANCE=envRadiance;
})();

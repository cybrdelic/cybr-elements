...
frames=root/'water-handprint-frames'
...
v=root.parent/'outputs'/'cybrdelic-handprint-water.mp4'
...
for name,indices in [('water-handprint-review.jpg',[30,60,120,180,240,315,345,375]),('water-handprint-release-review.jpg',[330,336,342,348,354,360,366,390])]:
...
report={'video':str(v),'metadata':s,'allFramesDecoded':True,'source':'handprint_motion.py','waterModel':'Reduced suspended liquid-jet dynamics, axial tension, viscous momentum exchange, cross-sectional volume conservation, breakup threshold, ballistic spray and gravity release. Not original FLIP solver.','renderer':'Blender Cycles OPTIX; 24 samples, refractive IOR 1.333; 1920x1080 native frames','fullViewHoldSeconds':3.55}
(root/'water-handprint-verification.json').write_text(json.dumps(report,indent=2))
print(json.dumps({'video':str(v),'metadata':s,'framesValidated':450,'decode':'passed','review':['water-handprint-review.jpg','water-handprint-release-review.jpg']}))

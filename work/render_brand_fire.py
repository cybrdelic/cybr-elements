...
if a.gesture_version=='23':
    ...
    gesture_pose=None
from brand_fire_motion import pose as nozzle_pose, turn_rate as nozzle_turn, WRITE, START, MODE, VARIANT, DATA
if not a.name.replace('-', '').isalnum(): raise ValueError('Expected a local artifact name')
out = ROOT/f'brand-fire-{VARIANT}-frames'
...
lo=np.array([-5.25,-.6,0],np.float32)
extent=np.array([10.5,1.2,5.8],np.float32)
...
def step(t):
    ...
    deposit=brand_supply*((arrival<=t)&(arrival>t-dt)).float()*.42
    torch.maximum(reservoir,deposit,out=reservoir)
    # The line stays supplied through the full-view hold. Closing the valve
    ...
    fuel.lerp_(torch.ones_like(fuel)*.22,gas_inject)
    ...
    angle=(temp*.8+soot*4).clamp(0,1)*(float(nozzle_turn(t))*dt)
...
FPS=30
SIM_FPS=a.fps
DURATION=float(os.environ.get('INTRO_SECONDS','15.0'))
TOTAL=round(DURATION*FPS)
W,H=1920,1080
video=ROOT.parent/'outputs'/f'cybrdelic-brand-{VARIANT}-fire.mp4'
video.parent.mkdir(exist_ok=True)
encoder=subprocess.Popen(['ffmpeg','-hide_banner','-loglevel','error','-y','-f','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r',str(FPS),'-i','-','-an','-c:v','libx264','-preset','fast','-crf','15','-pix_fmt','yuv420p','-movflags','+faststart',str(video)],stdin=subprocess.PIPE)
def render(frame):
    rgb,sigma=radiance()
    ...
    # Fade rising exhaust before it hits the top of the finite render domain.
    edge=((5.8-z[:,0,0])/.65).clamp(0,1)
    edge=edge*edge*(3-2*edge)
    linear=linear*edge[:,None,None]
    # A steady travelling camera frames the rolling wake. The fluid itself is untouched.
    physical=frame/SIM_FPS
    following=float(np.mean([nozzle_pose(physical+lag)[0][0] for lag in np.linspace(-1.2,.35,18)]))
    ...
    view_z=torch.linspace(2.35+view_height/2,2.35-view_height/2,H,device=device)
    vz,vx=torch.meshgrid(view_z,view_x,indexing='ij')
    lookup=torch.stack([(vx+5.25)/10.5*2-1,vz/5.8*2-1],dim=-1)[None]
    ...
    edge_x=((4.9-vx.abs())/.55).clamp(0,1)
    edge_z=torch.minimum((5.35-vz)/.60,vz/.12).clamp(0,1)
    edges=edge_x*edge_x*(3-2*edge_x)*edge_z*edge_z*(3-2*edge_z)
    # Fixed spatial camera-edge treatment only. No time-dependent image fade.
...
for frame in range(TOTAL):
    ...
    render(frame)
    if False:
        torch.save({'state':state,'reservoir':reservoir,'next_frame':frame+1,'size':a.size,'sim_fps':SIM_FPS,'substeps':a.substeps},ROOT/f'handprint-fire-checkpoint-{frame+1}.pt')
...
(ROOT/f'brand-fire-{VARIANT}-report.json').write_text(json.dumps({'frames':TOTAL,'fps':FPS,'size':[W,H],'solverGrid':a.size,'elapsed':time.monotonic()-started,'peakGpuMiB':torch.cuda.max_memory_allocated()/1048576,'source':'bake-reactive-fire.py','changes':'Gas supply stays open through screen t=11.0, giving 3.55 seconds in the fully zoomed-out view. After valve closure, finite gas inventory drains at the existing 1.8 per-second release rate; native fuel reaction, cooling and soot decay extinguish the fire. Constant exposure and opacity; no closing fade. Simulation checkpoints saved for future timing edits.'},indent=2))

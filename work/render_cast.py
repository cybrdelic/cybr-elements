...
if a.gesture_version=='23':
    ...
    gesture_pose=None
from cast_motion import emitters, turning, physical_time
if not a.name.replace('-', '').isalnum(): raise ValueError('Expected a local artifact name')
out = ROOT/'cast-final-frames'
...
def step(t):
    ...
    # Compact moving fuel nozzles. No stationary text mask or target field.
    v=state[0,:3]
    ...
    state[0,7]=burn/dt
    del burn,activation
    # Smaller letter jets use proportionally lower lift during the brief cast.
    ...
    angle=(temp*.8+soot*4).clamp(0,1)*(float(turning(t))*dt)
...
FPS=30
SIM_FPS=a.fps
DURATION=float(os.environ.get('INTRO_SECONDS','7.0'))
TOTAL=round(DURATION*FPS)
W,H=1920,1080
video=ROOT.parent/'outputs'/'cybrdelic-fire-cast.mp4'
...
def render(frame):
    ...
    rgb=im[0].permute(1,2,0).cpu().numpy()
    screen_z=4.2-(np.arange(H)-120)/768*4.2
    falloff=np.clip((3.8-screen_z)/.7,0,1)
    falloff=falloff*falloff*(3-2*falloff)
    horizontal=np.minimum(np.arange(W),W-1-np.arange(W))/65
...
for frame in range(TOTAL):
    t0=float(physical_time(frame/FPS));t1=float(physical_time((frame+1)/FPS))
    dt=(t1-t0)/a.substeps
...
(ROOT/'cast-report.json').write_text(json.dumps({'frames':TOTAL,'fps':FPS,'size':[W,H],'solverGrid':a.size,'elapsed':time.monotonic()-started,'peakGpuMiB':torch.cuda.max_memory_allocated()/1048576,'source':'bake-reactive-fire.py','changes':'Overlapping moving fuel nozzles and broad entrance/exit gestures; native reactive fluid transport and pressure; scaled lift during small letter jets; slow-motion timing and hot-gas exit force. No text overlay or stationary letter fuel mask.'},indent=2))

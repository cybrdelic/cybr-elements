...
FPS=30
SIM_FPS=a.fps
DURATION=float(os.environ.get('INTRO_SECONDS','10.0'))
TOTAL=round(DURATION*FPS)
W,H=1920,1080
video=ROOT.parent/'outputs'/'cybrdelic-gas-trail.mp4'
...
(ROOT/'gas-report.json').write_text(json.dumps({'frames':TOTAL,'fps':FPS,'size':[W,H],'solverGrid':a.size,'elapsed':time.monotonic()-started,'peakGpuMiB':torch.cuda.max_memory_allocated()/1048576,'source':'bake-reactive-fire.py','changes':'One moving nozzle deposits a persistent gas supply along its continuous path. Supply releases fuel until physical t=15.0, then stops by 15.7. Camera pulls back at screen t=5.55-7.45. Native combustion, pressure and transport remain active. No text overlay.'},indent=2))

...
const root=new URL('./',import.meta.url),out=new URL('cache/',root),route=JSON.parse(fs.readFileSync(new URL('path.json',root)));const mb=fs.readFileSync(new URL('mask.bin',root)),mask=new Float32Array(mb.buffer,mb.byteOffset,mb.byteLength/4);
...
class Directed extends FlipSolver{
 constructor(c){super(c);this.home=new Float32Array(this.maxParticles*3);this.tangent=new Float32Array(this.maxParticles*3);this.occupancy=new Map();}
 ...
 emit(dt){
  ...
  for(let i=0;i<this.count;i++){let q=i*3,g=this.target(i,this.time),next=this.target(i,this.time+.002),release=this.time>1.3+clamp((this.home[q]-.39)/3.9,0,1)*.42;
   for(let c=0;c<3;c++){let flow=release?clamp((next[c]-g[c])/.002,-12,12):this.tangent[q+c]*.045;let wave=c===2&&!release?.004*Math.sin(this.time*9+this.home[q]*3+this.home[q+1]*4):0;let force=160*(g[c]+wave-this.p[q+c])+22*(flow-this.v[q+c]);this.v[q+c]+=clamp(force,-90,90)*dt;}
  }
  ...
  for(let j=0;j<count;j++){let t=this.time+(end-this.time)*this.random(),sample=at(t);if(!sample.on)continue;let r=(this.random()-.5)*.12,z=(this.random()-.5)*.070,p=[sample.p[0]+nx*r,sample.p[1]+ny*r,sample.p[2]+z];if(!inside(p[0],p[1]))continue;let key=p.map(x=>Math.floor(x/this.h)).join(',');let used=this.occupancy.get(key)||0;if(used>=8)continue;
   let tangent=[dx/Math.max(len,1e-8),dy/Math.max(len,1e-8),0];let id=this.count;if(this.add(...p,...tangent.map(x=>x*.18))){this.home.set(p,id*3);this.tangent.set(tangent,id*3);this.occupancy.set(key,used+1);this.spawned++;}
  }
 }
}

...
a=water.node_tree.nodes.new('ShaderNodeVolumeAbsorption');a.inputs['Color'].default_value=(.65,.88,.92,1);a.inputs['Density'].default_value=.20;water.node_tree.links.new(a.outputs[0],water.node_tree.nodes.get('Material Output').inputs['Volume'])
...
tex=n.new('ShaderNodeNewGeometry');links.new(tex.outputs['Position'],noise.inputs['Vector']);ramp=n.new('ShaderNodeValToRGB');ramp.color_ramp.elements[0].color=(.09,.12,.14,1);ramp.color_ramp.elements[1].color=(.24,.28,.30,1);links.new(noise.outputs['Fac'],ramp.inputs[0]);links.new(ramp.outputs[0],n.get('Principled BSDF').inputs['Base Color'])
...
s.world.use_nodes=True;s.world.node_tree.nodes.get('Background').inputs[0].default_value=(.22,.25,.28,1);s.world.node_tree.nodes.get('Background').inputs[1].default_value=.45
...
out=ROOT/('proof' if '--no-spray' in sys.argv else 'frames');out.mkdir(exist_ok=True);start=time.time()
for f in frames:
 ...
 if '--no-spray' not in sys.argv:
  obs.append(spheres('Primary drops',coords(d['drops']),d['radii'],water));w=d['white'];obs.append(spheres('Secondary spray',coords(w[:,:3]),w[:,3],foam))

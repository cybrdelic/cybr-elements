...
const root=new URL('./',import.meta.url),out=new URL('cache-directed/',root),route=JSON.parse(fs.readFileSync(new URL('path.json',root)));const mb=fs.readFileSync(new URL('mask.bin',root)),mask=new Float32Array(mb.buffer,mb.byteOffset,mb.byteLength/4);
const config=makeProductionPreset('jets');Object.assign(config,{nameKey:'directed',h:.03,nx:360,ny:100,nz:40,extent:[10.8,3,1.2],obstacles:[],maxParticles:180000,seed:8107,gravity:[0,0,0]});
const waitCell=new Int32Array(new SharedArrayBuffer(4));function publish(){for(let i=0;;i++){try{fs.renameSync(new URL('manifest.tmp',out),new URL('manifest.json',out));return;}catch(e){if(!['EPERM','EBUSY','EACCES'].includes(e.code)||i>=199)throw e;Atomics.wait(waitCell,0,0,20);}}}
...
class Directed extends FlipSolver{
 ...
 target(i,t){let q=i*3,h=this.home,x=h[q],y=h[q+1],z=h[q+2],begin=1.3+(1-clamp((x-.39)/3.9,0,1))*.42,u=smooth((t-begin)/.65);return [x+5.4*u,y+(1.55+(y-1.7)*.3-y)*u+.20*Math.sin(Math.PI*u),z+(.66+(z-.60)*1.5+(x-2.34)*.04-z)*u+.07*Math.sin(Math.PI*u)];}
 emit(dt){
  // Explicit soft guide forces are applied before the original pressure projection.
  for(let i=0;i<this.count;i++){let q=i*3,g=this.target(i,this.time),next=this.target(i,this.time+.002),release=this.time>1.3+(1-clamp((this.home[q]-.39)/3.9,0,1))*.42;

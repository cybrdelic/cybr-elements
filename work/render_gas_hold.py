...
if a.gesture_version=='23':
    ...
    gesture_pose=None
from gesture_motion import pose as nozzle_pose, turn_rate as nozzle_turn, WRITE, START, MODE
if not a.name.replace('-', '').isalnum(): raise ValueError('Expected a local artifact name')
out = ROOT/'gas-hold-frames'
...
W,H=1920,1080
video=ROOT.parent/'outputs'/'cybrdelic-gas-hold.mp4'
...
# Preserve the already approved opening pixels while rebuilding its fluid state.

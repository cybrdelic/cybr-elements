...
out=ROOT/f'{element}-brand-{variant}-fastcheck';out.mkdir(exist_ok=True)
...
s.cycles.samples=32 if element=='water' else 24;s.cycles.use_denoising=True;s.cycles.use_adaptive_sampling=True;s.cycles.adaptive_threshold=.035 if element=='water' else .08;s.cycles.adaptive_min_samples=16 if element=='water' else 8
...
bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=2,radius=1);ico=bpy.context.object;base=np.array([v.co[:] for v in ico.data.vertices]);tri=np.array([p.vertices[:] for p in ico.data.polygons]);bpy.data.objects.remove(ico,do_unlink=True)
...
radii=rng.uniform(.007,.025,count) if element!='earth' else rng.uniform(.01,.055,count)
...
empty_frame=None
for frame in (range(240,243) if benchmark else [testframe] if pilot else range(450)):
...
(ROOT/f'{element}-brand-{variant}-report.json').write_text(json.dumps({'element':element,'variant':variant,'resolution':[s.render.resolution_x,s.render.resolution_y],'samples':s.cycles.samples,'triangles':len(faces),'frames':1 if pilot else 450,'elapsed':time.monotonic()-started,'model':'capillary sheet and ballistic droplets' if element=='water' else '2D advected condensation density with diffusion and evaporation, rendered as a heterogeneous volume' if element=='air' else 'suspended stone fragments with gravity release'},indent=2))

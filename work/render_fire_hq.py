...
if a.gesture_version=='23':
    ...
    gesture_pose=None
from intro_hq_motion import pose as nozzle_pose, turn_rate as nozzle_turn, WRITE, START, MODE
if not a.name.replace('-', '').isalnum(): raise ValueError('Expected a local artifact name')
out = ROOT/'fire-hq-frames'
...
def step(t):
    ...
    nozzle=torch.exp(-((across/.085)**2+(y/.100)**2+(along/.060)**2)*1.5)
...
FPS=30
SIM_FPS=a.fps
DURATION=float(os.environ.get('INTRO_SECONDS','9.5'))
TOTAL=round(DURATION*FPS)
W,H=1920,1080
video=ROOT.parent/'outputs'/'cybrdelic-fire-hq.mp4'

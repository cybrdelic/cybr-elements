...
if a.gesture_version=='23':
    ...
    gesture_pose=None
from font_motion import pose as nozzle_pose, turn_rate as nozzle_turn, WRITE, START, MODE
if not a.name.replace('-', '').isalnum(): raise ValueError('Expected a local artifact name')
out = ROOT/'script-fire-frames'
...
extent=np.array([10.5,1.2,4.2],np.float32)
...
W,H=1920,1080
video=ROOT.parent/'outputs'/'cybrdelic-script-fire.mp4'
...
def render(frame):
    ...
    edge_z=torch.minimum((3.75-vz)/.60,vz/.12).clamp(0,1)
...
for frame in range(TOTAL):
    ...
    if frame in [239,329,TOTAL-1]:
        torch.save({'state':state,'reservoir':reservoir,'next_frame':frame+1,'size':a.size,'sim_fps':SIM_FPS,'substeps':a.substeps},ROOT/f'script-fire-checkpoint-{frame+1}.pt')

...
R=Path(__file__).resolve().parent;B=R/'bending-rebuild-v5';OUT=B/'lightning-frames';OUT.mkdir(exist_ok=True)
W,H,FPS=1920,1080,30;cv2.setNumThreads(2)
EVENTS=np.array([.16,.327,.506,.744,.913,1.087,1.306,1.494,1.674,1.84])
...
def grow(event):
    ...
    rng=np.random.default_rng(18301+event*379);end=min(1.675,EVENTS[event]);start=max(.081,end-(.28+.065*(event%3)))
...
def exposure(f,event):
    ...
    leader=max(0,min(end,at)-max(t,at-.055))*FPS*.012
...
def render(f,nets,aerosol=None):
    ...
    if aerosol:linear+=scatter[:,:,None]*np.array([.26,.31,.43],np.float32)
...
if __name__=='__main__':
    ...
    if '--full' not in sys.argv:
        frames=[9,15,22,32,44,50];sheet=Image.new('RGB',(1440,810))
    ...
    aerosol=Aerosol();enc=subprocess.Popen(['ffmpeg','-v','error','-y','-f','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r','30','-i','-','-vf','scale=in_range=full:out_range=tv:out_color_matrix=bt709','-c:v','libx264','-threads','2','-preset','slow','-crf','15','-pix_fmt','yuv420p','-color_range','tv','-colorspace','bt709','-color_primaries','bt709','-color_trc','bt709','-movflags','+faststart',str(B/'lightning-v5.mp4')],stdin=subprocess.PIPE)
    for f in range(120):
        pixels=render(f,nets,aerosol);enc.stdin.write(pixels.tobytes())
        if f in [9,15,22,32,39,44,50,55,60,80,110]:Image.fromarray(pixels).resize((1280,720)).save(OUT/f'{f:04}.jpg',quality=95)

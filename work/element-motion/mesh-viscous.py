...
variant=sys.argv[1];folder=R/'viscous-cache';m=json.loads((folder/'manifest.json').read_text())
...
for f in frames:
 ...
 if n:
  ...
  config=dict(m['config']);config['surfaceOptions']={**config.get('surfaceOptions',{}),'spacingFactor':.4,'kernelRadiusFactor':1.08,'fieldSigma':.40,'meshSmoothingPasses':3};config['extent']=(upper-origin).tolist();rec=DetailReconstruction(config)

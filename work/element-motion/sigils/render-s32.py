...
dependencyNames=['render.py','materials.py','motion.py','botanical.py','channels.py',f'source-{V}.npz']
...
progressdir=R/'progress-s32';progressdir.mkdir(exist_ok=True);progressfile=progressdir/f'{K}-{V}.json'
...
s.camera.location.z=2.35;s.cycles.samples=32 if K not in ['sand','snow'] else 112
out=R/'frames'/f'{K}-{V}-s32';out.mkdir(parents=True,exist_ok=True)

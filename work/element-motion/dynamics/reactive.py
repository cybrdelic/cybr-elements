...
out = ROOT/'dynamics/frames/combustion'
...
lo=np.array([-5.25,-3. if MODE=='word' else -1.5,0],np.float32)
extent=np.array([10.5,6. if MODE=='word' else 3.0,4.2],np.float32)
...
def step(t):
    ...
    blast=1.68<t<1.755
    if blast:center=nozzle_pose(1.68)[0];on=1.;speed=5.
    elif t<1.68:on*=1.
    cx,cz=center;dx,dz=direction
    px=x-float(cx);pz=z-float(cz)
    across=-px*float(dz)+pz*float(dx);along=px*float(dx)+pz*float(dz)
    radius=.23 if blast else .085
    nozzle=torch.exp(-((px/radius)**2+(y/radius)**2+(pz/radius)**2)*1.5)
    if blast:nozzle*=torch.exp(-((across/.18)**2+(y/.16)**2)*.8);nozzle*=torch.clamp(.45+.36*torch.sin(px*29+pz*19+y*17)+.32*torch.sin(pz*39-y*23)*torch.cos(px*37-y*11),0,1)
    ...
    temp.lerp_(torch.ones_like(temp)*(2.8 if blast else 2.0),inject)
    v=state[0,:3]
    ...
    if blast:
        rr=torch.sqrt(px*px+y*y+pz*pz).clamp_min(.06);v[0].lerp_(px/rr*7+float(dx)*12+shear*1.1,inject);v[1].lerp_(y/rr*7+shear*1.1,inject);v[2].lerp_(pz/rr*7+float(dz)*12+shear*1.1,inject)
    activation=((temp-.15)/.22).clamp(0,1)
    burn=torch.minimum(fuel,oxygen*.7)*(1-math.exp(-22*dt))*activation
    fuel.sub_(burn); oxygen.sub_(burn/ .7).clamp_(0,1)
    temp.add_(burn*4.2).mul_(math.exp(-1.65*dt)).clamp_(0,3)
    soot.add_(burn*1.25).mul_(math.exp(-.52*dt))
    ...
    if 1.68<t<2.7:
        # Low-Mach thermal expansion: preserve a combustion-driven divergence target.
        # A strictly incompressible projection would cancel the radial burst.
        expansion=(state[0,7]*7).clamp(0,30);expansion=expansion-expansion.mean()
        spectral+=-1j*k*(torch.fft.rfftn(expansion)/k2)
...
def radiance():
    ...
    emission=colors*(front*1.1+soot*hot.pow(5)*.65)[...,None]
    light=torch.tensor([.020,.019,.025],device=device)+key[...,None]*torch.tensor([.80,.82,.87],device=device)
    rgb=emission+soot[...,None]*4.4*.42*(light+hot[...,None].pow(2)*torch.tensor([.8,.16,.015],device=device))/(4*math.pi)
    return rgb,sigma
...
W,H=1920,1080
video=ROOT/'dynamics/combustion-review.mp4'
...
def render(frame):
    ...
    cache_dir=ROOT/'dynamics/cache/combustion';cache_dir.mkdir(parents=True,exist_ok=True)
    fields=state[0,[5,6,7]].cpu().numpy(); fields[np.abs(fields)<.002]=0; np.savez_compressed(cache_dir/f'{frame:04}.npz',fields=fields.astype('<f2'))
    rgb,sigma=radiance()
    ...
    Image.fromarray(pixels).save(out/f'{frame:04}.jpg',quality=96)
...
(ROOT/'dynamics/combustion-report.json').write_text(json.dumps({'frames':TOTAL,'fps':FPS,'size':[W,H],'solverGrid':a.size,'elapsed':time.monotonic()-started,'peakGpuMiB':torch.cuda.max_memory_allocated()/1048576,'source':'sub-combustion.py','changes':'Short irregular fuel burst, reaction-driven divergence target for thermal expansion, turbulent transport and volume radiance. Low-Mach VFX approximation, not a compressible shock solver.'},indent=2))

...
R=Path(__file__).resolve().parent;full='--full' in sys.argv;O=R/'sigil-02-active-elements'/('water-full' if full else 'water-cpu')
...
commands=[['node',str(R/'sigil_02_active_water.mjs')]+(['--full'] if full else []),[sys.executable,str(R/'sigil_02_active_mesh.py')]+(['--full'] if full else ['--preview']),[blender,'--background','--python',str(R/'sigil_02_active_water_render.py'),'--']+(['--full'] if full else ['--preview'])]
processes=[];logs=[]
try:

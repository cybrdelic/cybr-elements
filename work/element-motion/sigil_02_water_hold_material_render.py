...
R=Path(__file__).resolve().parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=R/'sigil-02-water-hold/mesh';out=R/('sigil-02-water-hold/frames' if '--full' in sys.argv else 'sigil-02-water-hold/material-pilot');out.mkdir(exist_ok=True)
...
blackRay=wn.new('ShaderNodeMath');blackRay.operation='MAXIMUM';wl.new(lp.outputs['Is Camera Ray'],blackRay.inputs[0]);wl.new(lp.outputs['Is Transmission Ray'],blackRay.inputs[1]);wl.new(blackRay.outputs[0],mix.inputs[0]);bg.inputs['Strength'].default_value=.24
for light in bpy.data.lights:light.energy*=.44
# A small amount of volume scattering represents fine entrained air. The
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(300) if '--full' in args else [75,165]
for f in frames:

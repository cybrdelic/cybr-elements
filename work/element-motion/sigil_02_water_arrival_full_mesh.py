...
O=R/'sigil-02-water-arrival/full';source=O/'particles';out=O/'mesh';out.mkdir(exist_ok=True);pilot='--pilot' in sys.argv
preview='--preview' in sys.argv
if preview:out=O/'preview-mesh';out.mkdir(exist_ok=True)
frames=range(9,121,3) if preview else [12,24,36,48,60,75,90,120] if pilot else range(300);isolated=np.empty(0,bool);previous_ids=np.empty(0,np.uint32);previous_iso=1.8;begun=time.time()
...
for f in frames:
 ...
 if not pilot and not preview and f not in [24,48,75,90,120,165,200,240,299]:

...
O=R/'sigil-02-active-elements'/('water-full' if '--full' in sys.argv else 'water-cpu');source=O/'particles';out=O/'mesh';out.mkdir(exist_ok=True);pilot='--pilot' in sys.argv
preview='--preview' in sys.argv
if preview:out=O/'preview-mesh';out.mkdir(exist_ok=True)
frames=list(range(0,240,6)) if preview else [0,18,36,54,72,90,114,150,174] if pilot else range(240);isolated=np.empty(0,bool);previous_ids=np.empty(0,np.uint32);previous_iso=1.8;begun=time.time()
...
for f in frames:
 ...
 if n>2048:
  ...
  field,velocity,verts,normals,faces,drops,radii,dv,iso,measure=rec.reconstruct(p-origin,v,history,[],m['frameDt']);isolated=rec.isolated.copy();previous_iso=iso;verts+=origin;drops+=origin
  ...
  measure['unresolvedMarkerFraction']=float(isolated.mean());measure['renderedSubgridDrops']=len(drops)
  del field,velocity,history,tree,dist,nb,weights,rec

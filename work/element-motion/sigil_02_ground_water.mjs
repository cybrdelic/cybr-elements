...
const full=process.argv.includes('--full'),root=new URL(`sigil-02-bending-ground/${full?'full':'cpu'}/`,import.meta.url),out=new URL('particles/',root);fs.mkdirSync(out,{recursive:true});
...
const source=load('parcels.f32'),forces=[0,1,2].map(c=>load(`guide-${c}.f32`)),count=source.length/9;
...
function goal(i,t,result){
 const k=i*9,u=source[k+6],cz=source[k+7],cy=source[k+8];
 const a=smooth((t-.5-u*.55)/2.0),b=smooth((t-2.3-u*.5)/2.35);
 const mid=[-3.95+7.85*u,1.35+1.75*Math.sin(Math.PI*(u*.90+.08))+.28*cz,.75*Math.sin(u*Math.PI*2-.6)+.50*cy];
 for(let j=0;j<3;j++){
  const initial=source[k+j],m=mid[j]*.35+cfg.origin[j],target=source[k+3+j];
  result[j]=(initial+(m-initial)*a)*(1-b)+target*b;
 }
}
 ...
 step(dt){
  if(this.count){const t=this.time/cfg.timeScale,guide=1-smooth((t-5.1)/.75);
   if(guide>0)for(let i=0;i<this.count;i++){
    goal(i,t,this.goal0);goal(i,t+.025,this.goal1);
    for(let j=0;j<3;j++){const q=i*3+j,wanted=(this.goal1[j]-this.goal0[j])/(.025*cfg.timeScale);let acceleration=90*(this.goal0[j]-this.p[q])+14*(wanted-this.v[q]);acceleration=Math.max(-7,Math.min(7,acceleration));this.v[q]+=dt*guide*acceleration;}
   }
...
const sim=new Flow(c),manifest={config:c,origin:cfg.origin,spaceScale:cfg.spaceScale,timeScale:cfg.timeScale,frameDt:cfg.timeScale/30,playbackFps:30,frames:[],source:'All water exists at frame zero in a continuous ribbon on the floor. Authored external accelerations lift, bend and spread it into the sigil. Native FLIP and a solid floor solve the release. No timed reveal, outflow deletion or particle-position warp.',cacheFormat:'position-u16-velocity-i16-id-u32',cacheVelocityRange:16};
const started=performance.now();
for(let f=0;f<cfg.frames;f++){
 while(full&&fs.readdirSync(out).filter(n=>n.endsWith('.gz')).length>=80)await new Promise(r=>setTimeout(r,400));

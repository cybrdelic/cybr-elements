...
R=Path(__file__).resolve().parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=R/'sigil-02-water-hold/mesh';out=R/'sigil-02-water-hold/ripple-pilot';out.mkdir(exist_ok=True)
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=[120,165]
for f in frames:

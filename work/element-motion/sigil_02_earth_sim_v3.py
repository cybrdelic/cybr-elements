...
R=Path(__file__).resolve().parent;O=R/'sigil-02-elements';src=np.load(O/'earth-source-v3.npz')
...
np.savez_compressed(O/'earth-motion-v3.npz',transforms=cache,radii=r,births=born,velocity=v)
...
(O/'earth-sim-report-v3.json').write_text(json.dumps(report,indent=2));print(json.dumps(report))

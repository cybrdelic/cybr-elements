...
random.seed(6271);data=json.loads((R/'earth-geometry.json').read_text());out=R/'earth-material-frames';out.mkdir(exist_ok=True)
...
def material(name,color):
 ...
 noise=m.node_tree.nodes.new('ShaderNodeTexNoise');noise.inputs['Scale'].default_value=22;noise.inputs['Detail'].default_value=3;bump=m.node_tree.nodes.new('ShaderNodeBump');bump.inputs['Strength'].default_value=.28;bump.inputs['Distance'].default_value=.0015;m.node_tree.links.new(noise.outputs['Fac'],bump.inputs['Height']);m.node_tree.links.new(bump.outputs['Normal'],n.inputs['Normal']);ramp=m.node_tree.nodes.new('ShaderNodeValToRGB');ramp.color_ramp.elements[0].position=.2;ramp.color_ramp.elements[0].color=(*[c*.62 for c in color],1);ramp.color_ramp.elements[1].position=.8;ramp.color_ramp.elements[1].color=(*[min(1,c*1.5) for c in color],1);m.node_tree.links.new(noise.outputs['Fac'],ramp.inputs[0]);m.node_tree.links.new(ramp.outputs['Color'],n.inputs['Base Color']);return m
...
for k in range(180):
 ...
 me=bpy.data.meshes.new('Convex stone');me.from_pydata(vs,[],src['faces']);me.update();o=bpy.data.objects.new('Emitted stone '+str(k),me);bpy.context.collection.objects.link(o);o.data.materials.append(mats[k%len(mats)]);o.data.materials.append(mats[(k+2)%len(mats)]);
 for face in me.polygons:face.material_index=int((face.index*13+k*7)%5==0)
 bevel=o.modifiers.new('Small fractured edges','BEVEL');bevel.width=min(.006,r*.045);bevel.segments=2
 sub=o.modifiers.new('Fracture face tessellation','SUBSURF');sub.subdivision_type='SIMPLE';sub.levels=2;sub.render_levels=2
 texture=bpy.data.textures.new('Rock relief '+str(k),type='CLOUDS');texture.noise_scale=.035;texture.noise_depth=2
 displacement=o.modifiers.new('Stone face relief','DISPLACE');displacement.texture=texture;displacement.texture_coords='LOCAL';displacement.strength=min(.004,r*.025);displacement.mid_level=.52
 bpy.context.view_layer.objects.active=o;o.select_set(True);bpy.ops.rigidbody.object_add();o.select_set(False);rb=o.rigid_body;rb.mass=max(.01,src['volume']*(r/radius)**3*2700);rb.collision_shape='CONVEX_HULL';rb.mesh_source='BASE';rb.use_margin=True;rb.collision_margin=.001;rb.friction=.68;rb.restitution=.12;rb.linear_damping=.035;rb.angular_damping=.08
...
(R/'earth-material-report.json').write_text(json.dumps({'births':births,'frames':120,'fps':30,'trail':'shared-trail.json','lift':'Effective gravity .5 m/s2 during drawing, ramp to 9.81 after frame 51; Bullet free release and contacts','camera':DATA['camera']}))

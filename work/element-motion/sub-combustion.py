...
out = ROOT/'subelements/combustion-volume-frames'
...
def step(t):
    ...
    blast=1.55<t<1.64
    if blast:center=nozzle_pose(1.6)[0];on=1.;speed=5.
    elif t<1.55:on*=.3 if math.sin(t*38)>.5 else 0.
    cx,cz=center;dx,dz=direction
    px=x-float(cx);pz=z-float(cz)
    across=-px*float(dz)+pz*float(dx);along=px*float(dx)+pz*float(dz)
    radius=.24 if blast else .065
    nozzle=torch.exp(-((px/radius)**2+(y/radius)**2+(pz/radius)**2)*1.5)
    if blast:nozzle*=torch.clamp(.6+.22*torch.sin(px*31+y*19)*torch.sin(pz*27-y*23)+.18*torch.cos(px*53-pz*41),.05,1)
    ...
    temp.lerp_(torch.ones_like(temp)*(2.6 if blast else 1.5),inject)
    v=state[0,:3]
    ...
    if blast:
        rr=torch.sqrt(px*px+y*y+pz*pz).clamp_min(.06);v[0].lerp_(px/rr*11+shear*1.8,inject);v[1].lerp_(y/rr*11+shear*1.8,inject);v[2].lerp_(pz/rr*11+shear*1.8,inject)
    ...
    if 1.5<t<2.3:
        ...
        expansion=(state[0,7]*9).clamp(0,15);expansion=expansion-expansion.mean()
...
def radiance():
    ...
    emission=colors*(front*2.2)[...,None]
...
W,H=1920,1080
video=ROOT.parent.parent/'outputs/cybrdelic-type/elements/motion/subelements/combustion-volume.mp4'

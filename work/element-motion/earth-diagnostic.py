...
for f in range(1,121):
 s.frame_set(f)
 metrics.append({'frame':f,'hammer':list(hammer.matrix_world.translation),'pieces':[list(o.matrix_world.translation) for o in pieces]})
 if False:s.render.filepath=str(out/f'{f:04}.jpg');bpy.ops.render.render(write_still=True);print('FRAME',f,round(time.time()-start,1),flush=True)
...
(R/'earth-dynamics.json').write_text(json.dumps(metrics))

...
R=Path(__file__).resolve().parent;B=R/'bending-rebuild-v4';OUT=B/'lightning-frames';OUT.mkdir(exist_ok=True)
W,H,FPS=1920,1080,30;cv2.setNumThreads(2)
EVENTS=np.array([.19,.403,.713,.972,1.271,1.568,1.827])
CAM=np.array([1.1,-13.,2.8]);TARGET=np.array([0.,0.,1.903125]);FORWARD=TARGET-CAM;FORWARD/=np.linalg.norm(FORWARD)
...
def grow(event):
    dest=B/f'laplacian-forked-3d-{event:02}.npz'
    if dest.exists():
        with np.load(dest) as z:return {k:z[k] for k in z.files}
    rng=np.random.default_rng(18301+event*379);end=min(1.675,EVENTS[event]);start=max(.081,end-.89)
    guide=np.array([curve(t) for t in np.linspace(start,end,180)]);gtree=cKDTree(guide)
    stops=np.array([curve(t) for t in np.linspace(start,end,max(2,int(np.ceil((end-start)/.20))+1))])
    h=.052;radius=h*.5;cap=110000
    ...
    def add_candidates(index):
        ...
        if new:
            ...
            distance=gtree.query(positions)[0]
            # A wide control region allows meaningful diverging forks.
            corridor[new]=.07+.93*np.exp(-(distance/.62)**2)
    ...
    for step in range(4400):
        ...
        side=(stops[stage]+stops[stage-1])*.5+normal*(.80 if (stage+event)%2 else -1.0)+np.array([0,.32,0])
        ...
        weights=field**2.25*corridor[ids]**.55*np.exp(-np.minimum(d,ds)/1.20)
    ...
    roots=[j for j in range(1,len(p)) if not trunk[j] and trunk[parent[j]] and length[j]>.24]
        ...
        if level==1:
            for k in selected:
                for child in children[k]:
                    if child not in selected and length[child]>.22:light(child,amp*.22,2)
...
def exposure(f,event):
    at=EVENTS[event];t=f/FPS;end=t+1/FPS;main=fork=0.
    # Non-periodic return strokes reuse the established channel.
    for offset,power,duration in [(0,1.,.0048),(.036+event%3*.007,.58,.0035),(.112+event%2*.021,.22,.0028)]:
        e=max(0,min(end,at+offset+duration)-max(t,at+offset))*FPS
        main+=e*power;fork+=e*power*(1 if offset==0 else .035)
    leader=max(0,min(end,at)-max(t,at-.018))*FPS*.006
...
def channel(f,nets,w=W,h=H):
    ...
    for event,net in enumerate(nets):
        ...
        for j in np.flatnonzero(net['strength']):
            ...
            radius=(.29 if net['trunk'][j] else .12)*w/W
...
class Aerosol:
    ...
    def step(self,f,nets):
        on=pose(f/30)[2];center=curve(f/30);delta=self.world-center[:,None,None,None]
        nozzle=np.exp(-np.sum(delta*delta,axis=0)/.13**2)*self.noise
        ...
        for e,net in enumerate(nets):
            ...
            d=distance_transform_edt(mask,sampling=self.spacing);light+=energy*np.exp(-d/.6)/(.009+d*d)
        for _ in range(2):
...
def render(f,nets,aerosol=None):
    field,peak=channel(f,nets,W*2,H*2);core=cv2.resize(field,(W,H),interpolation=cv2.INTER_AREA)*145
    ...
    linear=core[:,:,None]*[1,.98,1]+near[:,:,None]*[.19,.19,.23]+halo[:,:,None]*[.014,.018,.042]+bloom[:,:,None]*[.003,.004,.012]
    if aerosol:linear+=scatter[:,:,None]*[.26,.31,.43]
...
if __name__=='__main__':
    started=time.time();nets=[grow(i) for i in range(len(EVENTS))]
    if '--full' not in sys.argv:
        frames=[12,21,29,38,47,55];sheet=Image.new('RGB',(1440,810))
        ...
        sheet=sheet.crop((0,0,1440,540));sheet.save(B/'lightning-pilot.jpg',quality=94);print('Pilot ready',flush=True);sys.exit(0)
    aerosol=Aerosol();enc=subprocess.Popen(['ffmpeg','-v','error','-y','-f','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r','30','-i','-','-vf','scale=in_range=full:out_range=tv:out_color_matrix=bt709','-c:v','libx264','-preset','slow','-crf','15','-pix_fmt','yuv420p','-color_range','tv','-colorspace','bt709','-color_primaries','bt709','-color_trc','bt709','-movflags','+faststart',str(B/'lightning-v4.mp4')],stdin=subprocess.PIPE)
    for f in range(120):
        pixels=render(f,nets,aerosol);enc.stdin.write(pixels.tobytes())
        if f in [12,21,29,38,47,55,60,80,110]:Image.fromarray(pixels).resize((1280,720)).save(OUT/f'{f:04}.jpg',quality=95)
        if f%20==0:print('FRAME',f,'seconds',round(time.time()-started,1),flush=True)
    enc.stdin.close();assert enc.wait()==0
    report=dict(model='3D point-charge Laplacian growth; authored guiding field; approximate exposed radiance and aerosol scattering',events=EVENTS.tolist(),frames=120,seconds=time.time()-started,aerosol=aerosol.rows,networks=[dict(nodes=len(n['points']),depthRange=float(np.ptp(n['points'][:,1])),finite=bool(np.isfinite(n['points']).all()),connected=bool(np.all(n['parent'][1:]<np.arange(1,len(n['parent']))))) for n in nets])

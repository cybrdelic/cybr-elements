...
R=Path(__file__).resolve().parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=R/'bending-rebuild-v4/water-mesh-calibrated';manifest=json.loads((cache/'manifest.json').read_text());out=R/'bending-rebuild-v4/water-final-frames';out.mkdir(exist_ok=True)
...
bpy.ops.object.camera_add(location=(2.8,-13,3.3));cam=bpy.context.object;cam.rotation_euler=(Vector((0,0,1.903125))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.type='PERSP';cam.data.lens=45;cam.data.sensor_width=36;s.camera=cam
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(120) if '--full' in args else [42]
for f in frames:

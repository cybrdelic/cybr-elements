...
const out=new URL('bending-rebuild-v4/water-particles/',import.meta.url);
...
class RibbonFlow extends FlipSolver{
 emit(dt){
  ...
  const width=.060*(1+.06*Math.sin(t*9)),radius=.058;
  ...
  for(let i=0;i<count;i++){
   ...
   const speed=1.90+.25*(1-rad*rad),omega=1.8+.4*Math.sin(t*9);
...
const manifest={config:c,frameDt:timeScale/30,playbackFps:30,timeScale,spaceScale,origin,frames:[],source:'Rounded 3D moving jet; native FLIP/APIC pressure and capillary solve; authored depth trajectory, lift and local turning; no mesh warp'};
...
for(let f=0;f<120;f++){
 ...
 if(f===42&&!process.argv.includes('--ungated')){
  ...
  while(!fs.existsSync(new URL('bending-rebuild-v4/continue-water',import.meta.url)))await new Promise(resolve=>setTimeout(resolve,500));
 }
}

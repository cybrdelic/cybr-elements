...
from bending_spray import Spray
B=R/'bending-rebuild-v3';source=B/'water-particles';out=B/'water-mesh-calibrated';out.mkdir(parents=True,exist_ok=True)
full='--full' in sys.argv;frames=range(120) if full else [42,75]
started=time.time();isolated=np.empty(0,bool);previous_iso=1.8
spray=None
for f in frames:
 if full:
  while len(list(out.glob('*.mesh.gz')))>=5:time.sleep(.5)
 while True:
 ...
 np.savez_compressed(out/f'{f:04}.velocity.npz',surface=vv,drops=dv)

...
random.seed(6271);data=json.loads((R/'earth-geometry.json').read_text());out=R/'bending-rebuild/earth-frames';out.mkdir(exist_ok=True)
...
for k in range(660):
 ...
 if family not in layerMeshes:
  ...
  for face,idx in zip(lm.polygons,surface['materials']):face.material_index=int(idx);face.use_smooth=True
...
for name,loc,power,size in [('Key',(-2,-4,3.8),650,3),('Rim',(1,2,4),850,3),('Fill',(2,-4,1.2),100,5)]:
...
for f in range(1,121):
 ...
 if f==51 and not full:
  cam.data.ortho_scale=4.5;cam.location.x=.8;cam.location.z=1.7;s.render.filepath=str(R/'bending-rebuild/earth-close.jpg');bpy.ops.render.render(write_still=True);cam.data.ortho_scale=10.5;cam.location.x=0;cam.location.z=1.903125
 if f%15==0:print('FRAME',f,'seconds',round(time.time()-start,1),flush=True)
(R/'bending-rebuild/earth-report.json').write_text(json.dumps({'births':births,'frames':120,'fps':30,'trail':'shared-trail.json','lift':'Effective gravity .5 m/s2 during drawing, ramp to 9.81 after frame 51; Bullet free release and contacts','camera':DATA['camera']}))

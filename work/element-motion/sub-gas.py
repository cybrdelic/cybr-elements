...
W,H=1920,1080
encoders={};folders={}
for kind in ['steam','smoke','flight','spirit-projection','energy','heat']:
...
def render(frame,kind):
    ...
    if kind in ['flight','spirit-projection','energy']:
        ...
        if kind=='flight':
            ...
            hot=state[0,5].clamp(0,1);rgb=density[...,None]*hot[...,None]*torch.stack([.15+.75*hot,.45+.3*hot,torch.ones_like(hot)],-1)*2.2
    ...
    if kind=='heat':
        ...
        optical=state[0,5].sum(1)*h[1];gx=(torch.roll(optical,-1,1)-torch.roll(optical,1,1))*.8;gz=(torch.roll(optical,-1,0)-torch.roll(optical,1,0))*.8
        xx=x[:,0];zz=z[:,0];warped=xx+gx;bands=(.5+.5*torch.sin(warped*22+gz*7)).pow(24)*.09

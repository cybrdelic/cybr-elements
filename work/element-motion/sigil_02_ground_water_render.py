...
R=Path(__file__).resolve().parent;full='--full' in sys.argv;O=R/'sigil-02-bending-ground'/('full' if full else 'cpu');cache=O/('preview-mesh' if '--preview' in sys.argv else 'mesh');out=O/('frames' if full else 'preview-frames' if '--preview' in sys.argv else 'pilot');out.mkdir(exist_ok=True)
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(390) if '--full' in args else list(range(0,390,6))+[389] if '--preview' in args else [0,18,42,66,90,114,144,180,234,270,300,354,389]
...
for f in frames:
 ...
 retreat=max(0,min(1,(f/30-8.7)/2.3));retreat=retreat*retreat*(3-2*retreat);cam.location=(.25,-17.2-2.0*retreat,5.1+.4*retreat);cam.rotation_euler=(Vector((0,0,1.55))-cam.location).to_track_quat('-Z','Y').to_euler()

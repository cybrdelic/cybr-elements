...
bpy.ops.object.camera_add(location=(.85,-17,3.04));cam=bpy.context.object;cam.rotation_euler=(Vector((0,0,2.8875))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.type='PERSP';cam.data.lens=44;cam.data.sensor_width=36;s.camera=cam
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(294)
for f in frames:

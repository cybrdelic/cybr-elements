...
random.seed(6271);data=json.loads((R/'earth-geometry.json').read_text());out=R/'earth-layered-frames';out.mkdir(exist_ok=True)
...
layers=json.loads((R/'layered-rocks.json').read_text());layerMeshes={}
rockMats=[material('Slate interior',(.07,.085,.10)),material('Fresh fracture',(.19,.18,.16)),material('Weathered seams',(.075,.047,.025)),material('Mineral inclusion',(.40,.36,.27))]
...
for name,loc,power,size in [('Key',(-2,-4,3.8),650,3),('Rim',(1,2,4),650,4)]:
...
for f in range(1,121):
 ...
 if f==51 and not full:
  cam.data.ortho_scale=4.5;cam.location.x=.8;cam.location.z=1.7;s.render.filepath=str(R/'earth-layered-close.jpg');bpy.ops.render.render(write_still=True);cam.data.ortho_scale=10.5;cam.location.x=0;cam.location.z=1.903125

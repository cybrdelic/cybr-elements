...
out = ROOT/'bending-rebuild-v5/air-frames'
...
def step(t):
    ...
    soot.lerp_(torch.ones_like(soot)*.7,inject*carrier.clamp(0,1));temp.zero_()
    v=state[0,:3]
...
SIM_FPS=a.fps
DURATION=4.0
TOTAL=46 if a.pilot else round(DURATION*FPS)
W,H=1920,1080
video=ROOT/'bending-rebuild-v5/air-v5.mp4'
video=video.with_name(video.stem+'-pilot.mp4') if a.pilot else video
video.parent.mkdir(exist_ok=True)
encoder=subprocess.Popen(['ffmpeg','-hide_banner','-loglevel','error','-y','-f','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r',str(FPS),'-i','-','-an','-c:v','libx264','-threads','2','-preset','fast','-crf','15','-pix_fmt','yuv420p','-movflags','+faststart',str(video)],stdin=subprocess.PIPE)
def render_baseline(frame):
    density=state[0,6]
    sigma=density.clamp_min(0).pow(1.45)*3.0
    ...
    rgb=sigma[...,None]*(lit*phase*4.2+.18)[...,None]*torch.tensor([.66,.76,.83],device=device)
    ...
    if True:
        Image.fromarray(pixels).resize((1280,720),Image.Resampling.LANCZOS).save(out/f'baseline-{frame:04}.jpg',quality=91)

def render(frame):
    density=state[0,6]
    sigma=density.clamp_min(0).pow(1.24)*3.0
    top=torch.flip(torch.cumsum(torch.flip(sigma,[0]),dim=0),[0])*h[2]
    back=torch.flip(torch.cumsum(torch.flip(sigma,[1]),dim=1),[1])*h[1]
    left=torch.cumsum(sigma,dim=2)*h[0]
    lit=torch.exp(-top*1.7-back*.8)
    side=torch.exp(-left*1.4-top*.65)
    cosine=(2-y)/torch.sqrt((x+2)**2+(2-y)**2+(z-4)**2).clamp_min(.1)
    phase=(1-.45**2)/(1+.45**2-2*.45*cosine).clamp_min(.05).pow(1.5)
    rgb=sigma[...,None]*(lit*phase*3.9+side*.85+.11)[...,None]*torch.tensor([.66,.76,.83],device=device)
    atten=torch.exp(-sigma*h[1])
    transmission=torch.cat([torch.ones_like(atten[:,:1]),torch.cumprod(atten[:,:-1],dim=1)],dim=1)
    linear=(transmission[...,None]*rgb*(1-atten[...,None])/sigma[...,None].clamp_min(1e-6)).sum(1)
    # Upsample the volume integration in linear light, then optical glow.
...
for frame in range(TOTAL):
    ...
    if frame in [20,45,65,100]:render_baseline(frame)
    render(frame)
    if frame==45:
        torch.cuda.empty_cache()
        print('REVIEW GATE: first 46 frames rendered',flush=True)
        while not (ROOT/'bending-rebuild-v5/continue-air').exists():time.sleep(.5)
encoder.stdin.close()
if encoder.wait()!=0:raise RuntimeError('Encoding failed')
(ROOT/'bending-rebuild-v5/air-report.json').write_text(json.dumps({'frames':TOTAL,'fps':FPS,'size':[W,H],'solverGrid':a.size,'elapsed':time.monotonic()-started,'peakGpuMiB':torch.cuda.max_memory_allocated()/1048576,'source':'air-filaments.py','simulationStepSha256':'e6522ffa34ee6095670a5770370836f5a049f325d848d8bc81753072127393a3','changes':'Moving tracer filaments with projected velocity and advection; extinction and directional volumetric lighting; no combustion or image fade.'},indent=2))

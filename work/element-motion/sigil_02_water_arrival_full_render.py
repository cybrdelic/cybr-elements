...
R=Path(__file__).resolve().parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=R/'sigil-02-water-arrival/full/mesh';out=R/('sigil-02-water-arrival/full/frames' if '--full' in sys.argv else 'sigil-02-water-arrival/full/pilot');out.mkdir(exist_ok=True)
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(300) if '--full' in args else [12,24,36,48,60,75,90,120]
for f in frames:

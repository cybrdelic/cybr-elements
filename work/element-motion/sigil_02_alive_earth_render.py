...
R=Path(__file__).resolve().parent;O=R/'sigil-02-alive';full='--full' in sys.argv
...
s.render.resolution_x=1920 if full else 960;s.render.resolution_y=1080 if full else 540;s.render.resolution_percentage=100;s.render.image_settings.file_format='JPEG';s.render.image_settings.quality=97;s.render.fps=30;s.frame_end=300;s.gravity=(0,0,-9.81);s.view_settings.view_transform='AgX'
...
for row in data['pieces']:
 ...
 # Neighbouring fragments share a travelling stress wave, with different
 ...
 objects.append(o)
# Released chips are irregular solid meshes with mass, gravity and Bullet
...
for f in range(1,301):
 ...
 if (full or f in [76,106,121,124,127,130,133,136,139,142,145,148,151,154,157,160,163,166]) and not (out/f'{f-1:04}.jpg').exists():
...
(O/('earth-report.json' if full else 'earth-pilot-report.json')).write_text(json.dumps(dict(frames=300,pieces=len(objects),method='Authored rigid assembly and correlated stress-wave hold; native Bullet collisions and gravity for released chips and final collapse; unchanged photogrammetry stone material',rows=rows),indent=2))

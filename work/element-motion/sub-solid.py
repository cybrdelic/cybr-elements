...
for idx,src in enumerate(data['pieces']):
 vs=[Vector(v) for v in src['verts']];centroid=sum(vs,Vector())/len(vs);rad=max((v-centroid).length for v in vs);vs=[list((v-centroid)/rad) for v in vs];faces=src['faces']
 if KIND=='crystal':
  vs=[[.45*math.cos(j*math.tau/6),.45*math.sin(j*math.tau/6),z] for z in [-.6,.6] for j in range(6)]+[[0,0,-1],[0,0,1]];faces=[(j,(j+1)%6,(j+1)%6+6,j+6) for j in range(6)]+[(12,(j+1)%6,j) for j in range(6)]+[(13,j+6,(j+1)%6+6) for j in range(6)]
 elif KIND=='glass':vs=[[v[0],v[1]*.25,v[2]] for v in vs]
 else:vs=[[v[0],v[1]*.6,v[2]] for v in vs]
 layers.append({'verts':vs,'faces':faces,'materials':[0]*len(faces)})
mat=bpy.data.materials.new(KIND);mat.use_nodes=True;nd=mat.node_tree.nodes;lk=mat.node_tree.links;pr=nd.get('Principled BSDF');pr.inputs['IOR'].default_value=1.31 if KIND=='ice' else 1.5;pr.inputs['Transmission Weight'].default_value=.85 if KIND=='ice' else 1;pr.inputs['Roughness'].default_value=.055 if KIND=='ice' else .012;pr.inputs['Base Color'].default_value=(.55,.8,.98,1) if KIND=='ice' else (.65,.22,.9,1) if KIND=='crystal' else (.92,.99,.96,1)
if KIND=='ice':
 no=nd.new('ShaderNodeTexNoise');no.inputs['Scale'].default_value=55;no.inputs['Detail'].default_value=3;bu=nd.new('ShaderNodeBump');bu.inputs['Distance'].default_value=.004;bu.inputs['Strength'].default_value=.35;lk.new(no.outputs['Fac'],bu.inputs['Height']);lk.new(bu.outputs['Normal'],pr.inputs['Normal'])
rockMats=[mat]
...
births=[];spawned=[]
pieceCount=360 if KIND=='ice' else 180
for k in range(pieceCount):
 b=4+int(k*47/pieceCount);t=(b-1)/30;p,d,on,speed=pose(t);r=min(.14,.04/(max(.035,random.random())**.48)) if KIND=='ice' else min(.185,.07/(max(.035,random.random())**.34));a=random.random()*math.tau;offset=random.random()**.5*.18
 ...
 if family not in layerMeshes:
  ...
  for face,idx in zip(lm.polygons,surface['materials']):face.material_index=0;face.use_smooth=False
...
for f in range(1,121):
 ...
 if f==51 and not full:
  cam.data.ortho_scale=4.5;cam.location.x=.8;cam.location.z=1.7;s.render.filepath=str(R/'subelements'/f'{KIND}-close.jpg');bpy.ops.render.render(write_still=True);cam.data.ortho_scale=10.5;cam.location.x=0;cam.location.z=1.903125
 if f%15==0:print('FRAME',f,'seconds',round(time.time()-start,1),flush=True)
(R/'subelements'/f'{KIND}-report.json').write_text(json.dumps({'births':births,'frames':120,'fps':30,'trail':'shared-trail.json','lift':'Effective gravity .5 m/s2 during drawing, ramp to 9.81 after frame 51; Bullet free release and contacts','camera':DATA['camera']}))

...
p=R/'sigil_02_repair_water_mesh_resume.py';s=p.read_text().replace('from bending_spray_v5 import Spray','from sigil_02_repair_spray import Spray')
...
# Lightning waits until the memory-heavy water pipeline has exited.
water=[start('water-sim-full',['node',str(R/'sigil_02_repair_water.mjs'),'--ungated']),start('water-mesh-full',[PY,str(R/'sigil_02_repair_water_mesh_resume.py')]),start('water-render-full',[BLENDER,'--background','--factory-startup','--python-exit-code','1','--python',str(R/'sigil_02_repair_water_render.py'),'--','--full'])]

...
ramp=wn.new('ShaderNodeValToRGB');cr=ramp.color_ramp;cr.elements.remove(cr.elements[1]);cr.elements[0].position=0;cr.elements[0].color=(.018,.028,.033,1)
for position,color in [(.15,(.04,.07,.08,1)),(.28,(.7,.8,.82,1)),(.34,(.12,.17,.18,1)),(.55,(.025,.035,.04,1)),(.85,(.20,.23,.25,1))]:
...
bpy.ops.object.camera_add(location=(.65,-16.8,3.15));cam=bpy.context.object;cam.rotation_euler=(Vector((0,0,1.8))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.type='PERSP';cam.data.lens=44;cam.data.sensor_width=36;s.camera=cam
# The fluid refracts the actual black backdrop. Reflection illumination must
# not turn that backdrop into bright opaque bands inside the water.
blackRay=wn.new('ShaderNodeMath');blackRay.operation='MAXIMUM';wl.new(lp.outputs['Is Camera Ray'],blackRay.inputs[0]);wl.new(lp.outputs['Is Transmission Ray'],blackRay.inputs[1]);wl.new(blackRay.outputs[0],mix.inputs[0]);bg.inputs['Strength'].default_value=.30
for light in bpy.data.lights:light.energy*=.65
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(240) if '--full' in args else [24,45,66,87]
for f in frames:
 ...
 if f==135:
  ...
  while not (R/'sigil-02-repair/continue-water').exists():time.sleep(.5)

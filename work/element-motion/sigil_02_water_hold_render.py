...
R=Path(__file__).resolve().parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=R/'sigil-02-water-hold/mesh';out=R/('sigil-02-water-hold/frames' if '--full' in sys.argv else 'sigil-02-water-hold/pilot');out.mkdir(exist_ok=True)
...
bpy.ops.object.camera_add(location=(.65,-16.8,4.2375));cam=bpy.context.object;cam.rotation_euler=(Vector((0,0,2.8875))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.type='PERSP';cam.data.lens=44;cam.data.sensor_width=36;s.camera=cam
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(300) if '--full' in args else [45,75,120,165]
for f in frames:

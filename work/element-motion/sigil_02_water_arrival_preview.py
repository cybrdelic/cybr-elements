...
R=Path(__file__).resolve().parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=R/'sigil-02-water-arrival/preview-mesh';out=R/('sigil-02-water-arrival/frames' if '--full' in sys.argv else 'sigil-02-water-arrival/preview-frames');out.mkdir(exist_ok=True)
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(300) if '--full' in args else range(9,121,3)
for f in frames:

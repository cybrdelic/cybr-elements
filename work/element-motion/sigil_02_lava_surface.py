...
def main():
 ...
 a=p.parse_args();settings=json.loads((a.run/'run.json').read_text());physics=settings['settings']['config'];floor=float(physics['floor']);contact_band=float(physics['spacing'])
 run=RunIdentity(a.out,dict(frames=a.frames,spacing=a.spacing,frame=a.frame,floor=floor,contactBand=contact_band),{'physics':a.run/'run.json',
 ...
 for f in ([a.frame] if a.frame is not None else range(a.frames)):
  ...
  while not source.exists():
   ...
   time.sleep(1)
  with np.load(source,allow_pickle=False) as s:mesh,row=reconstruct(dict(s),spacing=a.spacing,floor=floor,contact_band=contact_band)
 ...
 atomic_json(a.out/'manifest.json',dict(complete=True,frames=len(rows),spacing=a.spacing,floor=floor,contactBand=contact_band,elapsedSeconds=time.perf_counter()-start))

...
if a.gesture_version=='23':
    ...
    gesture_pose=None
from shared_motion import pose as nozzle_pose, turn_rate as nozzle_turn, WRITE, START, MODE
if not a.name.replace('-', '').isalnum(): raise ValueError('Expected a local artifact name')
out = ROOT/'air-filaments-frames'
...
def step(t):
    ...
    nozzle=torch.exp(-((across/(.17*(1+.18*math.sin(t*31))))**2+(y/.16)**2+(along/.10)**2)*1.5)
    ...
    fuel.lerp_(torch.ones_like(fuel)*.65,inject)
    oxygen.mul_(1-inject)
    carrier=torch.zeros_like(soot)
    for j in range(3):
        yc=(j-1)*.095+.018*math.sin(t*12+j*2);ac=.09*math.sin(t*9+j*2.1)
        carrier+=torch.exp(-((y-yc)/.028)**2-((across-ac)/.040)**2)
    soot.lerp_(torch.ones_like(soot)*.9,inject);temp.lerp_(torch.ones_like(temp)*.65,inject)
    v=state[0,:3]
    ...
    shear=3.2*torch.sin(y*19+t*13)*torch.cos((px+pz)*12-t*11)
    if MODE=='arc':
...
def radiance():
    ...
    emission=colors*(reaction.pow(.85)*4)[...,None]
...
def capture(frame,t,div):
    ...
    rgb=sigma[...,None]*depthLight[...,None]*torch.tensor([.66,.75,.80],device=device)*1.8
    ...
    im.save(folders[kind]/f'{frame:04}.jpg',quality=90)
...
W,H=1920,1080
encoders={};folders={}
for kind in ['energy','flight','heat']:
 video=ROOT.parent.parent/f'outputs/cybrdelic-type/elements/motion/subelements/{kind}.mp4';video.parent.mkdir(exist_ok=True);folders[kind]=ROOT/'subelements'/f'{kind}-frames';folders[kind].mkdir(exist_ok=True)
 encoders[kind]=subprocess.Popen(['ffmpeg','-hide_banner','-loglevel','error','-y','-f','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r',str(FPS),'-i','-','-an','-c:v','libx264','-threads','2','-preset','fast','-crf','17','-pix_fmt','yuv420p','-movflags','+faststart',str(video)],stdin=subprocess.PIPE)

def render(frame,kind):
    density=state[0,6]
    sigma=density.clamp_min(0).pow(1.2)*(2.5 if kind=='steam' else 6.0)
    top=torch.flip(torch.cumsum(torch.flip(sigma,[0]),dim=0),[0])*h[2]
    back=torch.flip(torch.cumsum(torch.flip(sigma,[1]),dim=1),[1])*h[1]
    lit=torch.exp(-top*1.7-back*.8)
    cosine=(2-y)/torch.sqrt((x+2)**2+(2-y)**2+(z-4)**2).clamp_min(.1)
    phase=(1-.45**2)/(1+.45**2-2*.45*cosine).clamp_min(.05).pow(1.5)
    rgb=sigma[...,None]*(lit*phase*4.2+.18)[...,None]*torch.tensor([.76,.84,.90] if kind=='steam' else [.22,.20,.18],device=device)
    if kind in ['flight','spirit-projection','energy']:
        # Material response changes; all structure still comes from the advected volume.
        if kind=='flight':
            rgb=rgb*torch.tensor([1.8,2.8,4.2],device=device);sigma*=.35
        elif kind=='spirit-projection':
            rgb=sigma[...,None]*(lit*.5+.5)[...,None]*torch.tensor([.16,.45,.8],device=device)*2
        else:
            hot=state[0,5].clamp(0,1);rgb=density[...,None]*(.4+hot)[...,None]*torch.stack([.15+.75*hot,.45+.3*hot,torch.ones_like(hot)],-1)*5
    ...
    if kind=='heat':
        # Integrated hot-gas density bends a background reflection pattern.
        optical=state[0,5].sum(1)*h[1];gx=(torch.roll(optical,-1,1)-torch.roll(optical,1,1))*3.5;gz=(torch.roll(optical,-1,0)-torch.roll(optical,1,0))*3.5
        xx=x[:,0];zz=z[:,0];warped=xx+gx;bands=(.5+.5*torch.sin(warped*22+gz*7)).pow(24)*.28
        envelope=torch.exp(-((xx/4.5)**8+((zz-2)/2.2)**8));linear=(bands*envelope)[...,None]*torch.tensor([.5,.6,.7],device=device)
    ...
    encoders[kind].stdin.write(pixels.tobytes())
    if frame%5==0 or frame==TOTAL-1:
        Image.fromarray(pixels).resize((1280,720),Image.Resampling.LANCZOS).save(folders[kind]/f'{frame:04}.jpg',quality=91)
for frame in range(TOTAL):
    ...
    if frame==45:
        np.savez_compressed(ROOT/'subelements/gas-material-pilot.npz',density=state[0,6].half().cpu().numpy(),temperature=state[0,5].half().cpu().numpy(),spacing=torch.as_tensor(h).cpu().numpy(),minimum=torch.as_tensor(lo).cpu().numpy(),extent=torch.as_tensor(extent).cpu().numpy())
    for kind in encoders:render(frame,kind)
for encoder in encoders.values():
 encoder.stdin.close()
 if encoder.wait()!=0:raise RuntimeError('Encoding failed')
(ROOT/'subelements/gas-refine-report.json').write_text(json.dumps({'frames':TOTAL,'fps':FPS,'size':[W,H],'solverGrid':a.size,'elapsed':time.monotonic()-started,'peakGpuMiB':torch.cuda.max_memory_allocated()/1048576,'source':'sub-gas.py','changes':'Shared moving source; three-dimensional advection, pressure projection and buoyancy. Steam and smoke scattering; luminous interpretations for energy and projection; integrated temperature refraction for heat.'},indent=2))

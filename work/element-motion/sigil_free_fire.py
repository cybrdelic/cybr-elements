...
if a.gesture_version=='23':
    ...
    gesture_pose=None
from sigil_free_motion import pose as nozzle_pose, turn_rate as nozzle_turn, WRITE, START, MODE, VARIANT
if not a.name.replace('-', '').isalnum(): raise ValueError('Expected a local artifact name')
out = ROOT/f'sigil-native/fire-{VARIANT}-free'
...
def step(t):
    ...
    gas_inject=(reservoir*(2.0*dt)).clamp(0,1)
    if t>=6.3:reservoir.sub_(gas_inject).clamp_min_(0)
...
TOTAL=255
W,H=1920,1080
video=ROOT/f'sigil-native/fire-{VARIANT}-free.mp4'

...
out = ROOT/'bending-rebuild-v5/fire-frames'
...
def radiance():
    temp=state[0,5]; soot=state[0,6]; reaction=state[0,7]
    hot=((temp-.28)/1.8).clamp(0,1)
    sigma=(soot*6.0+reaction*.032).clamp(0,16)
    tau=torch.flip(torch.cumsum(torch.flip(soot,[0]),dim=0),[0])*h[2]*4.4
    key=torch.exp(-tau)
    # Temperature controls hue; resolved reaction controls luminous structure.
    colors=torch.stack([torch.ones_like(hot),.035+.93*hot.pow(1.3),.002+.72*hot.pow(3.0)],dim=-1)
    front=reaction.clamp_min(0).pow(.95)/(1+.10*reaction.clamp_min(0))
    emission=colors*(front*7.0)[...,None]
    light=torch.tensor([.020,.019,.025],device=device)+key[...,None]*torch.tensor([1.4,1.10,.85],device=device)
    rgb=emission+soot[...,None]*4.4*.42*light/(4*math.pi)
    return rgb,sigma
...
W,H=1920,1080
video=ROOT/'bending-rebuild-v5/fire-v5.mp4'
...
for frame in range(TOTAL):
    ...
    if frame==45:
        ...
        while not (ROOT/'bending-rebuild-v5/continue-fire').exists():time.sleep(.5)
encoder.stdin.close()
if encoder.wait()!=0:raise RuntimeError('Encoding failed')
(ROOT/'bending-rebuild-v5/fire-report.json').write_text(json.dumps({'frames':TOTAL,'fps':FPS,'size':[W,H],'solverGrid':a.size,'elapsed':time.monotonic()-started,'peakGpuMiB':torch.cuda.max_memory_allocated()/1048576,'source':'bake-reactive-fire.py','simulationStepSha256':'b3182590b9c4180a9228895194de2ad688a53461d66a61d5499ad701b53289b0','changes':'Single moving fuel nozzle with backward jet momentum; speed-preserving hot-gas turning; native buoyancy, reaction, pressure and radiance; no trail fitting, morph or cached particles.'},indent=2))

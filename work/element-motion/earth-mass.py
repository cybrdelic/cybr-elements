...
random.seed(6271);data=json.loads((R/'earth-geometry.json').read_text());out=R/'earth-mass-frames';out.mkdir(exist_ok=True)
...
rockMats=[material('Slate interior',(.085,.065,.045)),material('Fresh fracture',(.11,.09,.06)),material('Weathered seams',(.025,.016,.009)),material('Mineral inclusion',(.18,.16,.12))]
...
for k in range(660):
 ...
 if k%55==0:r=random.uniform(.16,.23)
 elif k%3==0:r=random.uniform(.045,.09)
 else:r=min(.055,.012/(max(.045,random.random())**.65))
 ...
 if family not in layerMeshes:
  ...
  for face,idx in zip(lm.polygons,surface['materials']):face.material_index=0;face.use_smooth=True
  layerMeshes[family]=lm
 child=bpy.data.objects.new('Layered stone '+str(k),layerMeshes[family]);bpy.context.collection.objects.link(child);child.parent=o;child.scale=(r,r*.64,r) if k%55==0 else (r,r,r);child.hide_render=True;child.keyframe_insert('hide_render',frame=1);child.keyframe_insert('hide_render',frame=b-2);child.hide_render=False;child.keyframe_insert('hide_render',frame=b-1)
...
for f in range(1,121):
 ...
 if f==51 and not full:
  cam.data.ortho_scale=4.5;cam.location.x=.8;cam.location.z=1.7;s.render.filepath=str(R/'earth-mass-close.jpg');bpy.ops.render.render(write_still=True);cam.data.ortho_scale=10.5;cam.location.x=0;cam.location.z=1.903125
 if f%15==0:print('FRAME',f,'seconds',round(time.time()-start,1),flush=True)
(R/'earth-mass-report.json').write_text(json.dumps({'births':births,'frames':120,'fps':30,'trail':'shared-trail.json','lift':'Effective gravity .5 m/s2 during drawing, ramp to 9.81 after frame 51; Bullet free release and contacts','camera':DATA['camera']}))

...
R=Path(__file__).resolve().parent;data=json.loads((R/'earth-geometry.json').read_text());out=R/'earth-frames';out.mkdir(exist_ok=True)
...
for a,b in data['links']:
 bpy.ops.object.empty_add(location=(pieces[a].location+pieces[b].location)*.5);o=bpy.context.object;bpy.ops.rigidbody.constraint_add();c=o.rigid_body_constraint;c.type='FIXED';c.object1=pieces[a];c.object2=pieces[b];c.use_breaking=True;c.breaking_threshold=10+25*abs(math.sin(a*2+b));c.disable_collisions=True

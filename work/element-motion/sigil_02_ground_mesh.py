...
O=R/'sigil-02-bending-ground'/('full' if '--full' in sys.argv else 'cpu');source=O/'particles';out=O/'mesh';out.mkdir(exist_ok=True);pilot='--pilot' in sys.argv
preview='--preview' in sys.argv
if preview:out=O/'preview-mesh';out.mkdir(exist_ok=True)
frames=list(range(0,390,6))+[389] if preview else [0,18,42,66,90,114,144,180,234,270,300,354,389] if pilot else range(390);isolated=np.empty(0,bool);previous_ids=np.empty(0,np.uint32);previous_iso=1.8;begun=time.time()
...
for f in frames:
 ...
 if not pilot and not preview and f not in [0,66,114,180,234,270,300,354,389]:

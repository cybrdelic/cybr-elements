...
def encode(src,name,dust=False):
 ...
 if dust:args+=['-i',str(O/'earth-dust.mp4'),'-filter_complex','[0:v]format=gbrp[s];[1:v]format=gbrp[d];[s][d]blend=all_mode=screen:all_opacity=0.35,format=yuv420p']
...
if __name__=='__main__':
 import sys
 if sys.argv[1]=='earth':encode(R/'earth-mass-frames','earth-mass',True)

...
if a.gesture_version=='23':
    ...
    gesture_pose=None
from sigil_word_motion import pose as nozzle_pose, turn_rate as nozzle_turn, WRITE, START, MODE, VARIANT
if not a.name.replace('-', '').isalnum(): raise ValueError('Expected a local artifact name')
out = ROOT/f'sigil-native/fire-{VARIANT}-source'
...
lo=np.array([-5.25,-.6,-1.05],np.float32)
extent=np.array([10.5,1.2,5.90625],np.float32)
...
reservoir_dir=torch.zeros((2,*shape),device=device)
...
def step(t):
    ...
    nozzle=torch.exp(-((across/.105)**2+((y-.08*math.sin(t*17)-across*.35*math.sin(t*11))/.045)**2+(along/.075)**2)*1.5)
    ...
    # A thin gas line deposited by the nozzle. It replenishes fuel locally;
    # the reacting wake remains free, with the accepted renderer unchanged.
    line=torch.exp(-((across/.044)**2+(y/.05)**2+(along/.085)**2)*1.5)*on
    newly=line>reservoir
    ...
    gas_inject=(reservoir*(6.0*dt)).clamp(0,1)
    if t>=8.2:reservoir.sub_(gas_inject).clamp_min_(0)
    fuel.lerp_(torch.ones_like(fuel)*.35,gas_inject)
    oxygen.lerp_(torch.ones_like(oxygen)*.80,gas_inject)
    temp.lerp_(torch.ones_like(temp)*1.0,gas_inject)
    fuel.lerp_(torch.ones_like(fuel)*.95,inject)
    oxygen.mul_(1-inject)
    temp.lerp_(torch.ones_like(temp)*1.25,inject)
    v=state[0,:3]
    v[0].lerp_(reservoir_dir[0]*.15,(gas_inject*2).clamp(0,1));v[2].lerp_(reservoir_dir[1]*.15,(gas_inject*2).clamp(0,1))
    wake=torch.exp(-((across/.145)**2+(y/.18)**2+(along/.25)**2))*on
    ...
    angle=(temp*.8+soot*4).clamp(0,1)*wake*(float(nozzle_turn(t))*dt*.35 if MODE=='arc' else 0.)
...
TOTAL=181
W,H=1920,1080
video=ROOT/f'sigil-native/fire-{VARIANT}-source.mp4'
...
def render(frame):
    ...
    linear=linear # The simulation covers the full camera frustum; no internal crop.

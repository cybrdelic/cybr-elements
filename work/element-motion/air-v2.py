...
def step(t):
    ...
    nozzle=torch.exp(-((across/(.17*(1+.18*math.sin(t*31))))**2+(y/.16)**2+(along/.14)**2)*1.5)
    if MODE=='word':nozzle=torch.exp(-((px/.080)**2+((y+2.)/.100)**2+(pz/.080)**2)*1.5)
    inject=(nozzle*on*dt*(22+speed*4)).clamp(0,1)
    fuel.lerp_(torch.ones_like(fuel)*.65,inject)
    oxygen.mul_(1-inject)
    soot.lerp_(torch.ones_like(soot)*.6,inject);temp.zero_()
    v=state[0,:3]
    wake=torch.exp(-((across/.32)**2+(y/.3)**2+(along/.55)**2))*on
    v[2].add_(wake*torch.sin(y*18+t*15)*9*dt)
    v[1].add_(wake*torch.cos(pz*14-t*12)*8*dt)
...
W,H=1920,1080
video=ROOT.parent.parent/'outputs/cybrdelic-type/elements/motion/air-v2-test.mp4'

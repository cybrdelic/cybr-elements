...
from mathutils import Vector,noise as rocknoise
R=Path(__file__).resolve().parent;sys.path.insert(0,str(R));from shared_motion import pose,DATA,turn_rate
random.seed(6271);data=json.loads((R/'earth-geometry.json').read_text());out=R/('sigil-02-repair/earth-frames' if '--full' in sys.argv else 'sigil-02-repair/earth-pilot-detail');out.mkdir(exist_ok=True)
...
def material(name,color):
 ...
 coarse=m.node_tree.nodes.new('ShaderNodeTexNoise');coarse.inputs['Scale'].default_value=7;coarse.inputs['Detail'].default_value=2;macro=m.node_tree.nodes.new('ShaderNodeBump');macro.inputs['Strength'].default_value=.14;macro.inputs['Distance'].default_value=.016;m.node_tree.links.new(coarse.outputs['Fac'],macro.inputs['Height']);m.node_tree.links.new(bump.outputs['Normal'],macro.inputs['Normal']);m.node_tree.links.new(macro.outputs['Normal'],n.inputs['Normal']);# Large mineral patches survive the camera distance; fine noise alone did not.
...
for k,row in enumerate(source_rows):
 ...
 if family not in layerMeshes:
  ...
  # Physical surface relief on the shared high-resolution source meshes.
...
for f in range(1,193):
 ...
 if (full and not (out/f'{f-1:04}.jpg').exists()) or (not full and f in [46,67,91,145]):s.render.filepath=str(out/f'{f-1:04}.jpg');bpy.ops.render.render(write_still=True)
...
(R/'sigil-02-repair/earth-report.json').write_text(json.dumps({'births':births,'frames':192,'fps':30,'trail':'Full02 variable-width 3D source','lift':'Effective gravity .22 during bending, 9.81 on release after frame106. Bullet convex-hull contacts; no target springs.','camera':DATA['camera']}))

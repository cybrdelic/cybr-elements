...
for y in np.arange(-.38,.381,spacing):
 ...
 p=np.column_stack((a-.60,1.9+(b-1.9)*.78-.40,(cy+y)/.78))
 v=np.column_stack((np.full(len(a),.60/.65),.40/.80+(b-1.9)*.22/.70,-(cy+y)*(.22/.70)/(.78**2)))
...
report=dict(particles=len(a),grid=[cfg['nx'],cfg['ny'],cfg['nz']],sourceStart=float(a[:,0].min()),sourceEnd=float(a[:,0].max()),sourceSpeedMedian=float(np.median(np.linalg.norm(a[:,4:],axis=1))),initialUpstreamDistance=.60,holdUntil=5.8,render='Existing r3 water optics, unchanged')

...
function wave(x,t){
 ...
 const ox=-.95*a*Math.cos(3.4*age),oz=-.70*b*Math.cos(4*age),stretch=1-.35*d;
 const vx=.95*a*(Math.cos(3.4*age)/.65+3.4*Math.sin(3.4*age));
 const vz=.70*b*(Math.cos(4*age)/.80+4*Math.sin(4*age));
 return [age,ox,oz,stretch,vx,vz,.35/.70*d];
}
 ...
 applyForces(dt){
  ...
  for(let axis=0;axis<3;axis++){
   ...
   for(let i=0;i<=this.nx;i++){
    ...
    let tx=wx;for(let it=0;it<6;it++)tx=wx-wave(tx,t)[1];
    ...
    const strength=.52+.48*smooth(age/1.1),transport=1.8*Math.exp(-age/.85);
...
for(let f=0;f<end;f++){
 while(fs.readdirSync(out).filter(n=>n.endsWith('.gz')).length>=145)await new Promise(r=>setTimeout(r,300));

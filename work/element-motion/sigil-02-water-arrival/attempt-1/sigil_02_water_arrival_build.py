...
for y in np.arange(-.38,.381,spacing):
 ...
 birth=.25+(a+4.08)/8.16*1.85+.055*np.sin(a*2.1)
 ...
 p=np.column_stack((a-.95,1.9+(b-1.9)*.65-.70,(cy+y)/.65))
 v=np.column_stack((np.full(len(a),.95/.65),.70/.80+(b-1.9)*.35/.70,-(cy+y)*(.35/.70)/(.65**2)))
...
grad=np.gradient(sdf,h);norm=np.maximum(.1,np.sqrt(sum(g*g for g in grad)));outside=np.maximum(0,-sdf);band=np.exp(-(outside/.16)**2)
for axis in range(3):
...
report=dict(particles=len(a),grid=[cfg['nx'],cfg['ny'],cfg['nz']],sourceStart=float(a[:,0].min()),sourceEnd=float(a[:,0].max()),sourceSpeedMedian=float(np.median(np.linalg.norm(a[:,4:],axis=1))),initialUpstreamDistance=.95,holdUntil=5.8,render='Existing r3 water optics, unchanged')

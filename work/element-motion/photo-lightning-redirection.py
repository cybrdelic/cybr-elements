...
out=R/'subelements/photo-lightning-redirection-frames';out.mkdir(exist_ok=True)
...
for f in (range(120) if '--full' in sys.argv else [40,48]):
 ...
 if .12<t<2.25:
  ...
  pts.append(base[-1]);pts=list(reversed(pts));channel(pts,.004)

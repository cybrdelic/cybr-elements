...
random.seed(6271);data=json.loads((R/'earth-geometry.json').read_text());out=R/'earth-shared-frames';out.mkdir(exist_ok=True)
...
def material(name,color):
 m=bpy.data.materials.new(name);m.use_nodes=True;n=m.node_tree.nodes.get('Principled BSDF');n.inputs['Base Color'].default_value=(*color,1);n.inputs['Roughness'].default_value=.83;return m
mats=[material('Stone '+str(i),(.15+i*.015,.14+i*.013,.12+i*.01)) for i in range(5)];floorMat=material('Ground',(.008,.009,.010))
...
births=[]
for k in range(300):
 b=4+int(k*47/300);t=(b-1)/30;p,d,on,speed=pose(t);r=min(.115,.032/(max(.02,random.random())**.40));a=random.random()*math.tau;offset=random.random()**.5*.14
...
for name,loc,power,size in [('Key',(-2,-4,6),1600,5),('Rim',(1,2,4),2100,4)]:
...
(R/'earth-shared-report.json').write_text(json.dumps({'births':births,'frames':120,'fps':30,'trail':'shared-trail.json','lift':'Effective gravity .5 m/s2 during drawing, ramp to 9.81 after frame 51; Bullet free release and contacts','camera':DATA['camera']}))

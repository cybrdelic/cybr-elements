...
R=Path(__file__).resolve().parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=R/'sigil-02-repair/water-mesh';out=R/('sigil-02-repair/water-frames' if '--full' in sys.argv else 'sigil-02-repair/water-pilot');out.mkdir(exist_ok=True)
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(192) if '--full' in args else [24,45,66,87]
for f in frames:

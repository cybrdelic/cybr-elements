...
R=Path(__file__).resolve().parent;B=R/'sigil-02-elements/lightning-v2';B.mkdir(exist_ok=True);OUT=B/'frames';OUT.mkdir(exist_ok=True)
...
started=time.time();video=B.parent/'lightning-02-v2.mp4';enc=subprocess.Popen(['ffmpeg','-v','error','-y','-f','rawvideo','-pix_fmt','rgb24','-s','1920x1080','-r','30','-i','-','-c:v','libx264','-threads','2','-preset','fast','-crf','16','-pix_fmt','yuv420p','-movflags','+faststart',str(video)],stdin=subprocess.PIPE)
for f in range(294):
 ...
 if f==160:
  ...
  while not (B.parent/'continue-lightning-v2').exists():time.sleep(.5)
enc.stdin.close();assert enc.wait()==0
(B.parent/'lightning-report-v2.json').write_text(json.dumps({'frames':294,'networks':len(nets),'nodes':sum(len(n['points']) for n in nets),'elapsed':time.time()-started,'aerosol':aerosol.rows,'model':'Art-directed 3D branched conductor graphs, subtree current weighting, geodesic excitation, exposed return strokes and pressure-projected lit aerosol; not calibrated plasma'},indent=2));print('LIGHTNING COMPLETE',flush=True)

...
R=Path(__file__).resolve().parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=R/'sigil-02-water-hold/mesh';out=R/'sigil-02-water-hold/cool-pilot';out.mkdir(exist_ok=True)
...
for light in bpy.data.lights:
 light.energy*=.11
 light.color=(.18,.52,1.0)
for light_object in bpy.data.objects:
 if light_object.type=='LIGHT':light_object.visible_transmission=True
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=[165]
for f in frames:

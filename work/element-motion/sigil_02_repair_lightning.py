...
def grow(event):
    dest=B/f'discharge-{event:02}.npz'
    if dest.exists():
        with np.load(dest) as z:return {k:z[k] for k in z.files}
    rng=np.random.default_rng(18301+event*379)
    ...
    def add_candidates(index):
        ...
        if new:
            ...
            corridor[new]=.008+.992*np.exp(-(distance/(.065+widths[nearest]*.75))**2)
    ...
    for step in range(6500):
        ...
        phi=potential[ids]+(.75*(1+(step-stage_start)/400)**2)/np.maximum(d,.08)+.12/np.maximum(ds,.10)
        field=np.maximum(0,(phi-phi.min())/max(1e-10,phi.max()-phi.min()))
        weights=field**2.25*corridor[ids]**1.8*np.exp(-np.minimum(d,ds)/1.20)

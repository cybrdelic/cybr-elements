...
R=Path(__file__).resolve().parent;full='--full' in sys.argv;O=R/'sigil-02-water-whip'/('full' if full else 'cpu');cache=O/('preview-mesh' if '--preview' in sys.argv else 'mesh');out=O/('frames' if full else 'preview-frames' if '--preview' in sys.argv else 'pilot');out.mkdir(exist_ok=True)
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(390) if '--full' in args else list(range(0,180,6)) if '--preview' in args else [0,18,36,54,72,90,114,150,174]

...
R=Path(__file__).resolve().parent;O=R/'sigil-02-elements';src=np.load(O/'earth-source-v2.npz')
target=src['centers'];r=src['radii'];born=src['births'];direction=src['directions'];n=len(r)
rng=np.random.default_rng(92314);p=target-direction*.65;p[:,1]-=.55
v=direction*2.0;v[:,1]=1.1;omega=rng.normal(0,1.2,(n,3));rot=Rotation.random(n,random_state=rng).as_quat()
...
np.savez_compressed(O/'earth-motion-v2.npz',transforms=cache,radii=r,births=born,velocity=v)
...
(O/'earth-sim-report-v2.json').write_text(json.dumps(report,indent=2));print(json.dumps(report))

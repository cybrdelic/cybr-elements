...
class Spray:
    ...
    def step(self,p,v,isolated,dt,cluster):
        ...
        chosen=(~dense)&(energetic[ids]|self.active[ids])
        ...
        if len(new):
            ...
            fragmentCount=np.clip((4+20*weber[new]/(weber[new]+8)).astype(int),4,self.fragments)
            radii=np.exp(normal*.65)*(k<fragmentCount[:,None]);radii*=np.cbrt(self.volume*3/(4*np.pi)/np.sum(radii**3,axis=1))[:,None]

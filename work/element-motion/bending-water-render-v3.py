...
R=Path(__file__).resolve().parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=R/'bending-rebuild-v3/water-mesh-calibrated';manifest=json.loads((cache/'manifest.json').read_text());out=R/'bending-rebuild-v3/water-final-frames';out.mkdir(exist_ok=True)
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(120) if '--full' in args else [42,75]
for f in frames:

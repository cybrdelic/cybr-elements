...
R=Path(__file__).resolve().parent;O=R/'sigil-02-elements';out=O/'earth-frames';out.mkdir(exist_ok=True)
data=np.load(O/'earth-motion.npz');poses=data['transforms'];radii=data['radii'];birth=data['births']
...
frames=[60,135,180,235] if '--pilot' in sys.argv else range(294);started=time.time()
for f in frames:

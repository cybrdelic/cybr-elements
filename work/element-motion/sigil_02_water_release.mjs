...
const root=new URL('sigil-02-repair/',import.meta.url),out=new URL('water-release-particles/',root);fs.mkdirSync(out,{recursive:true});
...
class Flow extends FlipSolver{
 ...
 emit(dt){this.sourceCursor??=0;const t=(this.time+dt)/timeScale;while(this.sourceCursor<count&&source[this.sourceCursor*7]<=t){const k=this.sourceCursor++*7;if(this.add(...source.subarray(k+1,k+7))){this.ids[this.count-1]=k/7;this.spawned++;}}}
...
const started=performance.now(),total=216;
for(let f=0;f<total;f++){
 // Primary caches are retained so any subsequent review can reuse them.
 ...
 const n=sim.count;if(f>=24&&f<136&&n<1000)throw Error('Source validation: expected emitted water');const raw=Buffer.concat([Buffer.from(sim.p.buffer,0,n*12),Buffer.from(sim.v.buffer,0,n*12),Buffer.from(sim.ids.buffer,0,n*4)]);

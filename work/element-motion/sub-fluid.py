...
R=Path(__file__).resolve().parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=O/('water/cache/viscous' if KIND in ['lava','mud'] else 'water/cache/material');manifest=json.loads((cache/'manifest.json').read_text());out=R/'subelements'/f'{KIND}-frames';out.mkdir(parents=True,exist_ok=True)

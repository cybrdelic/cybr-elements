...
bpy.ops.mesh.primitive_plane_add(size=100);floor=bpy.context.object;floor.data.materials.append(floorMat);bpy.ops.rigidbody.object_add();floor.rigid_body.type='PASSIVE';floor.rigid_body.friction=.85;floor.location.z=-3;floor.hide_render=True;floor.select_set(False)
...
births=[];spawned=[]
for k in range(660):
 b=4+int(k*47/660);t=(b-1)/30;p,d,on,speed=pose(t)
 if k%24==0:r=random.uniform(.18,.29)
 elif k%3==0:r=random.uniform(.045,.09)
 else:r=min(.048,.009/(max(.045,random.random())**.65))
 a=random.random()*math.tau;offset=random.random()**.5*.14
 ...
 for attempt in range(45):
  angle=random.random()*math.tau;cross=random.random()**.5*.22;along=random.uniform(-.09,.09)
 ...
 child=bpy.data.objects.new('Layered stone '+str(k),layerMeshes[family]);bpy.context.collection.objects.link(child);child.parent=o;child.scale=(r,r*.78,r) if k%24==0 else (r,r,r);child.hide_render=True;child.keyframe_insert('hide_render',frame=1);child.keyframe_insert('hide_render',frame=b-2);child.hide_render=False;child.keyframe_insert('hide_render',frame=b-1)
...
for f in range(1,121):
 ...
 for sub in range(3):
  ...
  hit=active&(gp[:,2]<gr-3);gp[hit,2]=gr[hit]-3;gv[hit,2]=np.abs(gv[hit,2])*.12;gv[hit,:2]*=.68
 if grit:
 ...
 if (full and not (out/f'{f-1:04}.jpg').exists()) or (not full and f in [21,51,81]):s.render.filepath=str(out/f'{f-1:04}.jpg');bpy.ops.render.render(write_still=True)
 if f==51 and not full:
  cam.data.ortho_scale=4.5;cam.location.x=.8;cam.location.z=1.7;s.render.filepath=str(R/'bending-rebuild-v5/earth-close.jpg');bpy.ops.render.render(write_still=True);cam.data.ortho_scale=10.5;cam.location.x=0;cam.location.z=1.903125
 if f%15==0:print('FRAME',f,'seconds',round(time.time()-start,1),flush=True)
np.savez_compressed(R/'bending-rebuild-v5/earth-transforms.npz',transforms=np.asarray(transforms,dtype=np.float32))
(R/'bending-rebuild-v5/earth-report.json').write_text(json.dumps({'births':births,'frames':120,'fps':30,'trail':'shared-trail.json','lift':'Effective gravity .5 m/s2 during drawing, ramp to 9.81 after frame 51; Bullet free release and contacts','camera':DATA['camera']}))

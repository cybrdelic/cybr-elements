...
B=R/'sigil-02-repair';source=B/'water-release-particles';out=B/'water-release-mesh';out.mkdir(parents=True,exist_ok=True)
full=True;frames=range(216)
started=time.time();isolated=np.empty(0,bool);previous_iso=1.8
spray=None;previous_ids=np.empty(0,np.uint32);orphan_p=np.empty((0,3),np.float32);orphan_v=orphan_p.copy();orphan_r=np.empty(0,np.float32)
for f in frames:
 ...
 p=raw[:n*3].reshape(-1,3);v=raw[n*3:n*6].reshape(-1,3);particle_ids=raw[n*6:].view('<u4');extent=np.array(m['config']['extent']);h=m['config']['h']
 if f>=136 and spray is not None:
 ...
 if (B/'water-release-frames'/f'{f:04}.jpg').exists():
  ...
  pass # Retain primary cache for reproducible validation.
 ...
 if pilot and f not in [24,45,66,87]:
  pass # Retain primary cache for reproducible validation.;continue
 ...
 if not pilot:pass # Retain primary cache for reproducible validation.

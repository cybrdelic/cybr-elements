...
out = ROOT/'air-material-v2-frames'
...
def step(t):
    ...
    soot.lerp_(torch.ones_like(soot)*.28,inject);temp.zero_()
    v=state[0,:3]
...
W,H=1920,1080
video=ROOT.parent.parent/'outputs/cybrdelic-type/elements/motion/air-material-v2.mp4'
...
def render(frame):
    ...
    rgb=sigma[...,None]*(lit*phase*2.35+.18)[...,None]*torch.tensor([.66,.76,.83],device=device)

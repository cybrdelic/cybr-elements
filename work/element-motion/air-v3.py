...
if a.gesture_version=='23':
    ...
    gesture_pose=None
from motion import pose as nozzle_pose, turn_rate as nozzle_turn, WRITE, START, MODE
if not a.name.replace('-', '').isalnum(): raise ValueError('Expected a local artifact name')
out = ROOT/'air-v3-frames'
...
def step(t):
    ...
    center,direction,on,speed=([-2.8,1.9],[1.,0.],max(0.,min(1.,t/.06,(.48-t)/.06)),6.)
    cx,cz=center;dx,dz=direction
    px=x-float(cx);pz=z-float(cz)
    across=-px*float(dz)+pz*float(dx);along=px*float(dx)+pz*float(dz)
    nozzle=torch.exp(-(((across*across+y*y).sqrt()/.30)**8+(along/.12)**2)*1.5)
    if MODE=='word':nozzle=torch.exp(-((px/.080)**2+((y+2.)/.100)**2+(pz/.080)**2)*1.5)
    inject=(nozzle*on*dt*(45+speed*5)).clamp(0,1)
    fuel.lerp_(torch.ones_like(fuel)*.65,inject)
    oxygen.mul_(1-inject)
    soot.lerp_(torch.ones_like(soot)*.22,inject);temp.zero_()
    v=state[0,:3]
    wake=torch.exp(-((across/.32)**2+(y/.3)**2+(along/.55)**2))*on
    v[2].add_(wake*torch.sin(y*18+t*15)*1.2*dt)
    v[1].add_(wake*torch.cos(pz*14-t*12)*1.0*dt)
    ...
    angle=(temp*.8+soot*4).clamp(0,1)*(0. if MODE=='arc' else 0.)
...
W,H=1920,1080
video=ROOT.parent.parent/'outputs/cybrdelic-type/elements/motion/air-v3-test.mp4'

...
R=Path(__file__).resolve().parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=O/('water/cache/viscous' if KIND in ['lava','mud'] else 'water/cache/material');manifest=json.loads((cache/'manifest.json').read_text());out=R/'subelements'/f'refined-{KIND}-frames';out.mkdir(parents=True,exist_ok=True)
...
if KIND=='lava':
 ...
 at=n.new('ShaderNodeAttribute');at.attribute_name='Temperature';bb=n.new('ShaderNodeBlackbody');l.new(at.outputs['Fac'],bb.inputs['Temperature']);l.new(bb.outputs[0],p.inputs['Emission Color']);div=n.new('ShaderNodeMath');div.operation='DIVIDE';div.inputs[1].default_value=1450;l.new(at.outputs['Fac'],div.inputs[0]);pw=n.new('ShaderNodeMath');pw.operation='POWER';pw.inputs[1].default_value=8;l.new(div.outputs[0],pw.inputs[0]);gain=n.new('ShaderNodeMath');gain.operation='MULTIPLY';gain.inputs[1].default_value=.8;l.new(pw.outputs[0],gain.inputs[0]);l.new(gain.outputs[0],p.inputs['Emission Strength'])
 vor=n.new('ShaderNodeTexVoronoi');vor.feature='DISTANCE_TO_EDGE';vor.inputs['Scale'].default_value=23
 edge=n.new('ShaderNodeMapRange');edge.clamp=True;edge.inputs['From Min'].default_value=.012;edge.inputs['From Max'].default_value=.075;edge.inputs['To Min'].default_value=1;edge.inputs['To Max'].default_value=.012;l.new(vor.outputs['Distance'],edge.inputs['Value'])
 fresh=n.new('ShaderNodeMapRange');fresh.clamp=True;fresh.inputs['From Min'].default_value=1370;fresh.inputs['From Max'].default_value=1460;fresh.inputs['To Min'].default_value=0;fresh.inputs['To Max'].default_value=.8;l.new(at.outputs['Fac'],fresh.inputs['Value'])
 mask=n.new('ShaderNodeMath');mask.operation='MAXIMUM';l.new(edge.outputs['Result'],mask.inputs[0]);l.new(fresh.outputs['Result'],mask.inputs[1]);heat=n.new('ShaderNodeMath');heat.operation='MULTIPLY';l.new(mask.outputs[0],heat.inputs[0]);l.new(gain.outputs[0],heat.inputs[1]);l.new(heat.outputs[0],p.inputs['Emission Strength'])
...
for f in frames:
 ...
 if KIND=='lava':
  ...
  for vi,pt in enumerate(verts):
   _,pid,_=kd.find(Vector(pt));age=max(0,(f-births[pid])/30);tempattr.data[vi].value=800+650*np.exp(-age*.65)
  ...
  for pid in range(0,count,2):
   ...
   u/=np.linalg.norm(u);v=np.cross(normal,u);rad=rng.uniform(.018,.052)*min(1,age/.18);thick=.004;center=np.array(q)+normal*.004;off=len(cv);nside=5

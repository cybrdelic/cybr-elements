...
if a.gesture_version=='23':
    ...
    gesture_pose=None
MODE='arc'; START=.30; WRITE=3.60; VARIANT='02'
if not a.name.replace('-', '').isalnum(): raise ValueError('Expected a local artifact name')
out = ROOT/'sigil-02-elements/air-frames-v2'
...
lo=np.array([-7.,-.6,-1.05],np.float32)
extent=np.array([14.,1.2,7.875],np.float32)
...
x=torch.linspace(float(lo[0]),float(lo[0]+extent[0]),X,device=device)[None,None,:]
y=torch.linspace(float(lo[1]),float(lo[1]+extent[1]),Y,device=device)[None,:,None]
z=torch.linspace(float(lo[2]),float(lo[2]+extent[2]),Z,device=device)[:,None,None]
...
state[:,4]=1
source_fields=np.load(ROOT/'sigil-02-v2/source.npz')
...
sheet_depth=.050+.040*(halfwidth/.27).clamp(0,1).sqrt()
source_fields.close()
...
def step(t):
    ...
    # The original broad, tapered artwork supplies NEW gas. Existing fluid
    ...
    front=(sheet*leading*dt*20).clamp(0,1)
    sustained=(current*dt*4.0).clamp(0,1)
    inject=(front+sustained).clamp(0,1)
    # Passive tracer injected in thin striated layers; velocity/pressure are
    # still solved volumetrically. The mark is only the source support.
    filament=.25+.75*(.5+.5*torch.sin(x*45+z*33+y*29-t*5)).pow(3)
    soot.lerp_(torch.ones_like(soot)*.55,inject*filament)
    fuel.zero_();temp.zero_()
    v=state[0,:3]
    speed=.16+5.1*leading
    shear=4.8*torch.sin(y*19+t*13)*torch.cos((x+z)*12-t*11)*leading
    v[0].lerp_(tangent_x*speed-tangent_z*shear,inject)
    v[1].lerp_(.18+torch.sin(x*38+z*27+t*23)*(.12+.68*leading),inject)
...
TOTAL=294
W,H=1920,1080
video=ROOT/'sigil-02-elements/air-02-v2.mp4'
...
for frame in range(TOTAL):
    ...
    if frame==135:
        ...
        while not (ROOT/'sigil-02-elements/continue-air-v2').exists():time.sleep(.5)
encoder.stdin.close()
if encoder.wait()!=0:raise RuntimeError('Encoding failed')
(ROOT/'sigil-02-elements/air-report-v2.json').write_text(json.dumps({'frames':TOTAL,'fps':FPS,'size':[W,H],'solverGrid':a.size,'elapsed':time.monotonic()-started,'peakGpuMiB':torch.cuda.max_memory_allocated()/1048576,'source':'bending-air-v5 optics and projected passive tracer flow, full approved 02 source geometry','changes':'Full 02 source cross-section with thin passive tracer filaments; free pressure-projected transport and native directional volume optics. Source drains after 6.8 seconds.'},indent=2))

...
R=Path(__file__).resolve().parent;args=sys.argv[sys.argv.index('--')+1:];variant=args[0];assert variant in ['01','02'];B=R/f'sigil-v1/water-{variant}';cache=B/'meshes';out=B/'frames';out.mkdir(exist_ok=True)
...
for name,loc,power,size,sy in [('Long white strip',(-2,-2,5.8),520,5.5,.75),('Edge card',(4,1.4,2.5),800,1.4,4.5),('Soft rim',(-4,2,3.6),850,3.8,2.3),('Low bounce',(-1,-1,-1.3),80,4,.9),('Broad front card',(1,-4,3.4),95,4.6,3.2)]:
 d=bpy.data.lights.new(name,'AREA');d.energy=power;d.color=(.70,.85,1.0) if name=='Broad front card' else (1,1,1);d.shape='RECTANGLE';d.size=size;d.size_y=sy;o=bpy.data.objects.new(name,d);bpy.context.collection.objects.link(o);o.location=loc;o.rotation_euler=(Vector((0,0,2.95))-o.location).to_track_quat('-Z','Y').to_euler()
bpy.ops.object.camera_add(location=(.45,-18,3.10));cam=bpy.context.object;cam.rotation_euler=(Vector((0,0,2.95))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.type='PERSP';cam.data.lens=57;cam.data.sensor_width=36;s.camera=cam
...
obj=None;sprayObj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(300)
for f in frames:
 ...
 if len(verts):
  ...
  if me.shape_keys.animation_data and me.shape_keys.animation_data.action:
   for fc in me.shape_keys.animation_data.action.fcurves:
    for k in fc.keyframe_points:k.interpolation='LINEAR'
 t=f/30;zoom=max(0,min(1,(t-2.4)/1));zoom=zoom*zoom*(3-2*zoom);cam.data.lens=57*(1.08-.08*zoom)

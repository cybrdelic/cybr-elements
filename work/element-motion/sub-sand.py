...
random.seed(6271);data=json.loads((R/'earth-geometry.json').read_text());out=R/'subelements/sand-frames';out.mkdir(parents=True,exist_ok=True)
...
rockMats=[material('Slate interior',(.22,.135,.05)),material('Fresh fracture',(.11,.09,.06)),material('Weathered seams',(.025,.016,.009)),material('Mineral inclusion',(.18,.16,.12))]
...
for f in range(1,121):
 ...
 if f==51 and not full:
  cam.data.ortho_scale=4.5;cam.location.x=.8;cam.location.z=1.7;s.render.filepath=str(R/'subelements/sand-close.jpg');bpy.ops.render.render(write_still=True);cam.data.ortho_scale=10.5;cam.location.x=0;cam.location.z=1.903125
 if f%15==0:print('FRAME',f,'seconds',round(time.time()-start,1),flush=True)
(R/'subelements/sand-report.json').write_text(json.dumps({'births':births,'frames':120,'fps':30,'trail':'shared-trail.json','lift':'Effective gravity .5 m/s2 during drawing, ramp to 9.81 after frame 51; Integrated particle drag, gravity and floor impacts; no inter-particle contact solve','camera':DATA['camera']}))

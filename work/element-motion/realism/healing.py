...
out=R/'healing-frames';out.mkdir(exist_ok=True)
for f in frames:
 t=f/30;front=1.0
 for o in objects:
 ...
 for ob,v,h,scale,c,direction,side,depth,u in shoots:
  age=t-(.08+1.6*u);repair=float(np.clip(age/.7,0,1));g=1.0;ob.hide_render=False
  if ob.hide_render:continue
  z=np.maximum(0,v[:,2]);unfurl=np.clip((g-z/h)*4+.4,0,1);unfurl=unfurl*unfurl*(3-2*unfurl);axial=np.minimum(z,h*g);sway=.018*np.sin(t*3+u*20)*(z/h)**2;vv=c+axial[:,None]*direction*scale+(v[:,0]*unfurl*scale)[:,None]*side+(v[:,1]*unfurl*scale+sway)[:,None]*depth;vv+=((axial/h)**2*.025*scale)[:,None]*np.array([0,0,-1]);
...
(R/'healing-report.json').write_text(json.dumps({'asset':'Poly Haven nettle_plant CC0','shoots':len(shoots),'sharedTrail':True,'samples':s.cycles.samples,'frames':list(frames)}))

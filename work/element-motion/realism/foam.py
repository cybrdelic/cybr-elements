...
R=Path(__file__).resolve().parent.parent;O=R.parent.parent/'outputs/cybrdelic-type/elements/motion';cache=O/('water/cache/viscous' if KIND in ['lava','mud','foam'] else 'water/cache/material');manifest=json.loads((cache/'manifest.json').read_text());out=R/'realism'/f'{KIND}-frames';out.mkdir(parents=True,exist_ok=True)
...
if KIND=='lava':
 ...
 p.inputs['Base Color'].default_value=(.023,.016,.008,1);p.inputs['Roughness'].default_value=.48;p.inputs['Coat Weight'].default_value=.22;bump.inputs['Distance'].default_value=.028;noise.inputs['Scale'].default_value=48;bump.inputs['Strength'].default_value=.6
elif KIND=='foam':
 ...
 if KIND=='blood':
  p.inputs['Base Color'].default_value=(.07,.001,.003,1);p.inputs['Transmission Weight'].default_value=.1;p.inputs['Roughness'].default_value=.21;p.inputs['Coat Weight'].default_value=.3;absorb.inputs['Color'].default_value=(.45,.002,.006,1);absorb.inputs['Density'].default_value=14
...
# Continuous scattering foam bulk with a wet microcell surface.
n=m.node_tree.nodes;l=m.node_tree.links;n.clear();outm=n.new('ShaderNodeOutputMaterial');p=n.new('ShaderNodeBsdfPrincipled');l.new(p.outputs[0],outm.inputs['Surface']);p.inputs['Base Color'].default_value=(.62,.68,.70,1);p.inputs['Roughness'].default_value=.35;p.inputs['Subsurface Weight'].default_value=.22;p.inputs['Subsurface Radius'].default_value=(.012,.012,.012);p.inputs['Transmission Weight'].default_value=.10;p.inputs['IOR'].default_value=1.333;p.inputs['Coat Weight'].default_value=.32;p.inputs['Coat Roughness'].default_value=.075
cell=n.new('ShaderNodeTexVoronoi');cell.inputs['Scale'].default_value=210;cell.distance='EUCLIDEAN';ramp=n.new('ShaderNodeValToRGB');ramp.color_ramp.elements[0].position=.20;ramp.color_ramp.elements[0].color=(0,0,0,1);ramp.color_ramp.elements[1].position=.52;ramp.color_ramp.elements[1].color=(1,1,1,1);l.new(cell.outputs['Distance'],ramp.inputs[0]);bump=n.new('ShaderNodeBump');bump.inputs['Strength'].default_value=.7;bump.inputs['Distance'].default_value=.006;l.new(ramp.outputs[0],bump.inputs['Height']);l.new(bump.outputs[0],p.inputs['Normal'])
s.cycles.samples=128;s.cycles.adaptive_threshold=.025
wn=s.world.node_tree.nodes;wl=s.world.node_tree.links;tex=wn.new('ShaderNodeTexEnvironment');tex.image=bpy.data.images.load(str(R/'realism/assets/studio_small_08_2k.exr'));wl.new(tex.outputs[0],env.inputs['Color']);env.inputs['Strength'].default_value=.6
bubble=bpy.data.materials.new('Wet foam films');bubble.use_nodes=True;bs=bubble.node_tree.nodes.get('Principled BSDF');bs.inputs['Base Color'].default_value=(1,1,1,1);bs.inputs['Transmission Weight'].default_value=1;bs.inputs['IOR'].default_value=1.333;bs.inputs['Roughness'].default_value=.035
foamObj=None;obj=None;start=time.time();args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [];frames=range(120) if '--full' in args else [20,42,90]
for f in frames:
 ...
 if KIND=='foam':
  ...
  rawp=gzip.decompress((R/'viscous-cache'/f'{f:04}.gz').read_bytes());count=len(rawp)//24;pp=np.frombuffer(rawp,'<f4',count*3).reshape(-1,3);bp0=coords(pp)
  from mathutils.bvhtree import BVHTree
  tree=BVHTree.FromPolygons([Vector(v) for v in verts[:nv]],faces[:nf].tolist(),all_triangles=True);centers=[];rs=[]
  for pid,point in enumerate(bp0):
   pos,normal,index,distance=tree.find_nearest(Vector(point))
   if pos is not None and distance<.075:
    u=(np.sin(pid*127.1)*43758.5453)%1;radius=min(.044,.013/(1-u*.97)**.5)
  ...
  bp=np.array(centers).reshape(-1,3);br=np.array(rs)
  # Weighted contact planes flatten shared faces into packed cells.
  ...
  bv=(packed+bp[:,None]).reshape(-1,3);bf=(ifa[None]+np.arange(len(bp))[:,None,None]*len(iv)).reshape(-1,3)
  inner=(packed*.996+bp[:,None]).reshape(-1,3);bf=np.concatenate((bf,bf[:,[0,2,1]]+len(bv)));bv=np.concatenate((bv,inner))
  bm=bpy.data.meshes.new('Aerated cells');bm.from_pydata(bv.tolist(),[],bf.tolist());bm.materials.append(bubble);foamObj=bpy.data.objects.new('Foam cells',bm);bpy.context.collection.objects.link(foamObj)

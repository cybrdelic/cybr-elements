...
out = ROOT/'realism/combustion-frames'
...
def step(t):
    ...
    radius=.36 if blast else .075
    nozzle=torch.exp(-((px/radius)**2+(y/radius)**2+(pz/radius)**2)*1.5)
    if blast:nozzle*=torch.clamp(.45+.36*torch.sin(px*29+pz*19+y*17)+.32*torch.sin(pz*39-y*23)*torch.cos(px*37-y*11),0,1)
    ...
    if blast:
        rr=torch.sqrt(px*px+y*y+pz*pz).clamp_min(.06);v[0].lerp_(px/rr*16+shear*1.5,inject);v[1].lerp_(y/rr*16+shear*1.5,inject);v[2].lerp_(pz/rr*16+shear*1.5,inject)
    ...
    if 1.68<t<2.7:
        ...
        expansion=(state[0,7]*18).clamp(0,30);expansion=expansion-expansion.mean()
...
W,H=1920,1080
video=ROOT.parent.parent/'outputs/cybrdelic-type/elements/motion/subelements/rebuild-combustion.mp4'

...
ROOT=Path(__file__).parent/'lava-focus/mpm/rebuild-31'
...
def initial(pitch):
    ...
    for i,s in enumerate(seeds):
        ...
        poly=s+(poly-s)*rng.uniform(.973,.994)
        ...
        c=poly.mean(0);thick=rng.uniform(.023,.043)
        ...
        vv=[]
        for scale,z in [(.96,base),(1.,base+thick*.36),(.965,base+thick)]:
            ring=c+(poly-c)*scale
    ...
    # Pre-existing smaller clinkers collect at the advancing end. They are
    # released above the larger fragments and settle through rigid contact.
    for _ in range(12):
        xy=rng.uniform([.04,-.07],[.15,.07])
        if (xy[0]/.22)**2+(xy[1]/.125)**2>.92:continue
        rad=min(.022,.007*(1+rng.pareto(1.7)))
        top=.025
        for r in rocks:
            vv=r['v']+r['q'][:3]
            if np.linalg.norm(xy-r['q'][:2])<np.max(np.ptp(vv[:,:2],axis=0))*.6+rad:top=max(top,vv[:,2].max())
        directions=rng.normal(size=(22,3));directions/=np.linalg.norm(directions,axis=1)[:,None]
        vv=directions*rng.uniform(.7,1.,(22,1))*np.array([rad,rad*.8,rad*.65]);origin=np.r_[xy,top+rad*.7+.001]
        hull=ConvexHull(vv);ff=hull.simplices.copy();norm=np.cross(vv[ff[:,1]]-vv[ff[:,0]],vv[ff[:,2]]-vv[ff[:,0]])
        swap=np.sum(norm*hull.equations[:,:3],axis=1)<0;ff[swap]=ff[swap][:,[0,2,1]]
        worldh=ConvexHull(vv+origin);rocks.append(dict(v=vv,f=ff,q=np.r_[origin,[0,0,0,1]],equations=worldh.equations,base_temperature=880.))
...
def run(name,pitch,until,wall,basis):
    ...
    with wp.ScopedDevice('cuda:0'):
        ...
        rs=SolverXPBD(rigid,iterations=20,rigid_contact_con_weighting=True,rigid_contact_relaxation=.6)
        ...
        skin=wp.full((n,16),732220.,dtype=float);sn=wp.empty_like(skin);ts=wp.full(n,1450.,dtype=float)
        ...
        np.savez_compressed(folder/'rocks.npz',**geometry,initial_q=np.array([r['q'] for r in rocks]),mass=rigid.body_mass.numpy(),base_temperature=np.array([r.get('base_temperature',1420.) for r in rocks]))
        start=time.monotonic();clock=0.;rows=[];step=0;initialmass=n*2700*pitch**3;lossj=0.
        # Thermal preparation of the declared initial scene: eight seconds
        # held in place, then released mechanically. This is explicitly an
        # initial-condition solve, not an animated or simulated flow history.
        grid.build(fluid.particle_q,2.6*pitch)
        for pre in range(4000):
            wp.launch(heat_step,n,inputs=[fluid.particle_q,h,hn,grid.id,pitch,1.6,.94,12.,293.15,1173.15,.002,1,area,loss,cond])
            wp.launch(surface_heat,n,inputs=[h,hn,skin,sn,area,loss,ts,pitch,.002]);h,hn=hn,h;skin,sn=sn,skin
            lossj+=float(loss.numpy().sum(dtype='f8'))
            if pre%1000==999:
                values=ts.numpy()
                if not np.isfinite(values).all() or values.min()<293:raise RuntimeError('Invalid initial skin solve')
                print(json.dumps(dict(thermalPreparationSeconds=(pre+1)*.002,skinMinimumK=float(values.min()),wallSeconds=time.monotonic()-start)),flush=True)
        def save():
            ...
            np.savez_compressed(folder/f'frame-{round(clock*1000):06d}.npz',x=xx,v=vv,rest=x,h=h.numpy(),temperature=tf.numpy(),surface_temperature=ts.numpy(),body_q=q,body_qd=qd,pitch=pitch,time=clock,**extra)
            balance=float(h.numpy().sum(dtype='f8')*2700*pitch**3)-initialmass*732220.+lossj
            row=dict(time=clock,maximumFluidSpeed=float(np.linalg.norm(vv,axis=1).max()),maximumBodySpeed=float(np.linalg.norm(qd[:,:3],axis=1).max()),minimumBodyHeight=float(q[:,2].min()),fluidHeightRange=[float(xx[:,2].min()),float(xx[:,2].max())],thermalLossJ=lossj,thermalBalanceJ=balance,skinTemperatureRange=[float(ts.numpy().min()),float(ts.numpy().max())],wallSeconds=time.monotonic()-start)
            rows.append(row);print(json.dumps(row),flush=True)
        save()
        while clock<until-1e-8:
            if time.monotonic()-start>wall:break
            dt=min(.002,until-clock)
            for sub in range(4):
                ra.clear_forces()
            ...
            wp.launch(surface_heat,n,inputs=[h,hn,skin,sn,area,loss,ts,pitch,dt]);skin,sn=sn,skin
            ...
            if step%125==0 or clock>=until-1e-8:save()
        report=dict(status='complete' if clock>=until-1e-8 else 'bounded-preview',time=clock,particles=n,rocks=nb,pitch=pitch,velocityBasis=basis,fluidMassKg=initialmass,bodyMassKg=float(rigid.body_mass.numpy().sum()),rows=rows,seconds=time.monotonic()-start,sourceSha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),method='Newton implicit viscous MPM, XPBD rigid contact, native bidirectional collider impulses, particle enthalpy conduction/radiation, sixteen-cell conservative subgrid skin heat model.',limitations='Pre-existing basalt geometry and initial thermal state are authored. No new fracture formation, melting of rigid bodies, rock-fluid heat exchange or smoke solver. Skin temperature is subgrid; skin mechanics are unresolved. Finite release; nominal constitutive values; no convergence claim.')

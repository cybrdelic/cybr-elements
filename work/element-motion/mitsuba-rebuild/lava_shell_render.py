"""CPU Mitsuba views of separate molten and cohesive-crust meshes."""
...
def main(frame=59,spp=24,width=640,view='hero',revision='restored-clean',normal_only=False,no_micro=False):
 start=time.time();source=R/(os.environ.get('LAVA_SOURCE') or 'cohesive/shell-01.npz');a=np.load(source);v=a['v'];f=a['f'];normal=a['normal'];uv=a['rest'][:,:2]*4.;temperature=a['temperature'][f].mean(1);out=R/'renders';out.mkdir(exist_ok=True);geo=R/f'geometry-{revision}';geo.mkdir(exist_ok=True)
 ...
 if view=='detail':center=[-.14,-.03,.19];eye=[.38,-1.34,1.10];fov=42.
 ...
 for group in np.unique(groups):
  ...
  if solid>.45 and os.environ.get('LAVA_SURFACE_MIX') and not os.environ.get('LAVA_MATTE'):

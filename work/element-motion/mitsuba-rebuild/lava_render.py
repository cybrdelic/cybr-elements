...
def main(frame=59,spp=24,width=640,view='hero',revision='v2',normal_only=False,no_micro=False):
 ...
 for _ in range(12):normal=.4*normal+.6*(average@normal)
 ...
 weights={}
 for name,position,size,color in [('key',[-1.8,1.5,.15],[1.0,.3,1],[4.2,4.5,4.8]),('fill',[1.0,.4,-.4],[.18,.85,1],[.06,.055,.05])]:
 ...
 for group in np.unique(groups):
  select=groups==group;temp=float(temperature[select].mean());path=geo/f'lava-{group}.ply';ply(path,v,f[select],normal,uv);solid=float(np.clip((1330-temp)/240,0,1));alpha=.14+.26*solid

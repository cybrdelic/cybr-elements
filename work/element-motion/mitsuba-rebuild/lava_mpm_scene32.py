...
os.environ.setdefault('LAVA_MPM_LINEAR_BACKEND','cpu')
os.environ.setdefault('LAVA_MPM_FRICTION_BACKEND','cpu')
os.environ['CUDA_VISIBLE_DEVICES']='0' if os.environ['LAVA_MPM_LINEAR_BACKEND']=='cuda_dense' or os.environ['LAVA_MPM_FRICTION_BACKEND']=='cuda_batched' else '-1'
os.environ.update(LAVA_MPM_CONTACT_ACCELERATION='anderson',LAVA_MPM_CONTACT_WARM_START='1')
...
def initial(age=600.,spacing=.0025,source_interval=.05,vertical=1.):
    sample=np.array([spacing,spacing,spacing*vertical]);cell=sample*2
    # Resolve a continuous cooled shell through several particle layers.
    radii=np.array([.045,.030,.0225]);plane=-.045
    x=block([plane,-.030,0],[.045,.030,.0225],sample)
    radial=np.linalg.norm(x/radii,axis=1)
    conduit=(x[:,0]<-.020)&(abs(x[:,1])<.015)&(x[:,2]<.0125)
    x=x[((radial<1)&(x[:,0]>=-.025))|conduit];radial=np.linalg.norm(x/radii,axis=1)
    ...
    tube_depth=np.minimum(.015-abs(x[:,1]),.0125-x[:,2])
    depth=np.maximum(shell_depth,np.where(x[:,0]<-.020,tube_depth,-1.))
    ...
    reservoir=np.clip((-.020-x[:,0])/.020,0,1)
    ...
    config=dict(plane=plane,half_width=.015,height=.0125,peak_speed=.008,temperature=1450.,pipe_end=-.025)
    origin=np.array([-.075,-.065,-.015]);high=np.array([.135,.065,.100]);shape=np.ceil((high-origin)/cell).astype(int)+1
    ...
    setup=dict(sceneKind='resolved_continuous_crust_under_renewed_feed',initialCondition='Stress-free 9 x 6 x 2.25 cm continuous lobe, zero damage and zero cracks. 2.5 mm isotropic particles resolve the cooled shell through its thickness. Temperature comes from a 1D cooling solve mapped by depth; prior 3D thermal stress history is not represented.',
        thermalProfile=thermal,reservoirInitialization='Enthalpy blends to 1450 K across x=-20 to -40 mm in the upstream pipe; fluid inlet does not grip a cold solid plug.',config=config,period=source_interval,nextEmission=source_interval,emissions=0,
...
def run(name,until,wall,dt,age=600.,spacing=.0025,source_interval=.05,vertical=1.):
    folder=ROOT/'rebuild-32'/name;folder.mkdir(parents=True,exist_ok=True)
    ...
    start=time.monotonic();error=None;steps=0;next_frame=(np.floor(s.time/.05)+1)*.05
        ...
        while s.time<until-1e-9:
            ...
            while True:
                ...
                try:row=s.step(step,bed=HotBed(1450.),boundary=setup['config']);break
                    ...
                    print(json.dumps(dict(rejectedTime=s.time,dt=step,error=str(exc))),flush=True)
                    step*=.5
                    if step<1e-7:raise
            steps+=1
            print(json.dumps(dict(acceptedTime=s.time,dt=step,damage=float(s.damage.max()),broken=int(s.connectivity.broken.sum()),coupling=row.get('damageCouplingIterations'),friction=row.get('frictionContact'),wallSeconds=time.monotonic()-start)),flush=True)
            if s.time>=next_frame-1e-9:
                s.save(folder/f'frame-{round(s.time*1000):04d}');next_frame+=.05
                print(json.dumps(dict(time=s.time,damage=float(s.damage.max()),broken=int(s.connectivity.broken.sum()),coherent=int(s.connectivity.frozen.sum()),maximumSpeed=float(np.linalg.norm(s.v,axis=1).max()),wallSeconds=time.monotonic()-start)),flush=True)
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--name',default='continuous-crust');p.add_argument('--until',type=float,default=.1);p.add_argument('--wall',type=float,default=180);p.add_argument('--dt',type=float,default=.01);p.add_argument('--age',type=float,default=600.);p.add_argument('--spacing',type=float,default=.0025);p.add_argument('--source-interval',type=float,default=.05);p.add_argument('--vertical',type=float,default=1.)

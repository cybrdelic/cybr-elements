"""Mitsuba spectral CPU hero views of the lava-only material study."""
...
def main(frame=59,spp=24,width=640,view='hero'):
 ...
 scene={'type':'scene','integrator':{'type':'aov','aovs':'albedo:albedo,normal:sh_normal,depth:depth','beauty':{'type':'path','max_depth':12,'rr_depth':6}},'sensor':{'type':'perspective','to_world':cam,'fov':fov,'sampler':{'type':'independent','sample_count':spp},'film':{'type':'hdrfilm','width':width,'height':height,'pixel_format':'rgba','rfilter':{'type':'tent'}}}}
 ...
 groups=np.floor(temperature/22).astype(int)
 for group in np.unique(groups):
  select=groups==group;temp=float(temperature[select].mean());path=geo/f'lava-{group}.ply';ply(path,v,f[select],normal,uv);solid=float(np.clip((1330-temp)/240,0,1));alpha=.12+.20*solid
  ...
  material={'type':'bumpmap','scale':.00032*solid,'texture':{'type':'bitmap','filename':str(R/'basalt-micro.png'),'raw':True},'material':material}
  scene[f'lava_{group}']={'type':'ply','filename':str(path),'bsdf':material}
  if temp>800:scene[f'lava_{group}']['emitter']={'type':'area','radiance':{'type':'blackbody','temperature':temp}}
 scene=mi.load_dict(scene);render_start=time.time();mi.render(scene,spp=spp,seed=319);bitmap=scene.sensors()[0].film().bitmap(raw=True);parts={name:np.array(bmp) for name,bmp in bitmap.split()};names={name:list(value.shape) for name,value in parts.items()};print('AOV',json.dumps(names),flush=True)
 beauty=parts.get('beauty',parts.get('<root>'));assert beauty is not None;hdr=np.ascontiguousarray(beauty[...,:3]);albedo=parts.get('albedo');shnormal=parts.get('normal');assert np.isfinite(hdr).all();stem=out/f'{view}-{frame:04}-{width}-{spp}spp';mi.Bitmap(hdr).write(str(stem.with_suffix('.exr')));render_seconds=time.time()-render_start;np.savez_compressed(str(stem)+'-raw-aov.npz',color=hdr,albedo=albedo,normal=shnormal);dn_start=time.time();clean=denoise(hdr,albedo,shnormal);denoise_seconds=time.time()-dn_start
 # Preserve exact black outside the path-traced scene. No environment,
 # gradient backdrop or painted haze is added.
 background=np.max(np.abs(hdr),axis=-1)==0;clean[background]=0;exposure=5.0
 ...
 receipt={'renderer':'Mitsuba '+mi.__version__,'variant':mi.variant(),'device':'CPU','threads':dr.thread_count(),'affinity':proc.cpu_affinity(),'samplesPerPixel':spp,'size':[width,height],'renderSeconds':round(render_seconds,2),'denoiseSeconds':round(denoise_seconds,2),'totalSeconds':round(time.time()-start,2),'denoiser':'OIDN, explicitly CPU; GPU backends disabled','exposure':exposure,'background':'exact black','radianceQuantiles':np.quantile(hdr,[.5,.95,.99,.999,1]).tolist(),'meshSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'imageSha256':hashlib.sha256(stem.with_suffix('.png').read_bytes()).hexdigest(),'status':'candidate; direct visual review required'};stem.with_suffix('.json').write_text(json.dumps(receipt,indent=2));print('LAVA_IMAGE',json.dumps(receipt),flush=True)
if __name__=='__main__':
 ap=argparse.ArgumentParser();ap.add_argument('--frame',type=int,default=59);ap.add_argument('--spp',type=int,default=24);ap.add_argument('--width',type=int,default=640);ap.add_argument('--view',choices=['hero','detail'],default='hero');a=ap.parse_args();assert a.width<=1600 and a.spp<=128;main(a.frame,a.spp,a.width,a.view)

...
def transport(x,v,temp,near,count,h,dt,reference_neighbors):
 ...
 for i in range(n):
  surface[i]=min(1.,max(0.,1-count[i]/reference_neighbors));eta=min(100000.,1500*np.exp(20000*(1/max(850.,temp[i])-1/1450.)));coeff[i]=dt*eta/(2700*h*h)*6
 ...
 for iteration in range(24):
  for ii in range(n):
   i=ii if iteration%2==0 else n-1-ii
   if x[i,2]<.025:v[i,0]=0.;v[i,1]=0.;v[i,2]=0.;new[i,:]=0.;continue
   ...
   new[i,0]=s0/total;new[i,1]=s1/total;new[i,2]=s2/total;v[i,:]=new[i,:]
  v[:]=new

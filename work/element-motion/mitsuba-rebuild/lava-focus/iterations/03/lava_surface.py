...
def main(frame=59,detail=.004):
 ...
 phase=material[:,0]*85+3.3*np.sin(material[:,1]*10)+2.2*warp+.3*large
 ridge=(.5+.5*np.sin(phase))**2.7;fine_ridge=(.5+.5*np.sin(phase*2.07+medium*.8))**3
 wrinkle=(.003+.013*comp)*ridge+.0012*fine_ridge;rough=.0005*medium+.00015*small
 ...
 opening=np.maximum(opening*.45,primary*top)
 lip=np.exp(-((qtear-1.04)/.19)**2)*reach*top*.011
 molten_roll=.012*(.5+.5*np.sin(material[:,0]*36+warp*.7))**2*opening**2

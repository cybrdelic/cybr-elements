...
def main(frame=59,spp=24,width=640,view='hero',revision='v2'):
 ...
 for name,position,size,color in [('key',[-1.1,.75,-.6],[1.0,.3,1],[24,25,26]),('fill',[1.0,.4,-.4],[.18,.85,1],[6,5.5,5])]:
 ...
 for group in np.unique(groups):
  select=groups==group;temp=float(temperature[select].mean());path=geo/f'lava-{group}.ply';ply(path,v,f[select],normal,uv);solid=float(np.clip((1330-temp)/240,0,1));alpha=.12+.27*solid
  ...
  material={'type':'bumpmap','scale':.00010*solid,'texture':{'type':'bitmap','filename':str(R/'basalt-micro.png'),'raw':True},'material':material}
  scene[f'lava_{group}']={'type':'ply','filename':str(path),'bsdf':material}
  if temp>1150:scene[f'lava_{group}']['emitter']={'type':'area','radiance':{'type':'rgb','value':radiance(temp)}}
...
if __name__=='__main__':
 ap=argparse.ArgumentParser();ap.add_argument('--frame',type=int,default=59);ap.add_argument('--spp',type=int,default=24);ap.add_argument('--width',type=int,default=640);ap.add_argument('--view',choices=['hero','detail'],default='hero');ap.add_argument('--revision',default='v2');a=ap.parse_args();assert a.width<=1600 and a.spp<=128;main(a.frame,a.spp,a.width,a.view,a.revision)

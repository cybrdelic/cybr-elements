...
def main(frame=59,detail=.004):
 ...
 phase=(material[:,0]+.42*(material[:,1]/.34)**2)*62+.65*warp
 ridge=(.5+.5*np.sin(phase))**1.6;fine_ridge=(.5+.5*np.sin(phase*3.07+medium*.35))**3
 wrinkle=(.015+.036*comp)*ridge+.0016*fine_ridge;rough=.00035*medium+.00010*small
 ...
 opening=np.maximum(opening*.22,primary*top)
 ...
 lean=.011*np.cos(phase)*topness*(1-opening)

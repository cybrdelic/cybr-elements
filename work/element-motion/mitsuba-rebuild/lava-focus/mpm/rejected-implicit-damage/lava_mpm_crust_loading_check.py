...
def main(a):
    ...
    if (folder/'state.json').exists():s=MPM.load(folder)
        ...
        initial=dict(source=str(source/'state.npz'),sourceSha256=hashlib.sha256((source/'state.npz').read_bytes()).hexdigest(),
            ...
            gripSpeedMPerS=a.speed,maxDt=a.dt,implicitDamage=a.implicit_damage,seededDamage=False,prescribedCracks=False)
    ...
    while s.time<setup['time']+a.duration-1e-9:
        ...
        if a.implicit_damage:
            from lava_mpm_damage_corrector import step as implicit_step
            implicit_step(s,dt,bed=bed,node_velocity=grips)
        else:s.step(dt,bed=bed,node_velocity=grips)
        n+=1
...
if __name__=='__main__':
    ...
    p.add_argument('--duration',type=float,default=.08);p.add_argument('--speed',type=float,default=.001);p.add_argument('--wall',type=float,default=95);p.add_argument('--dt',type=float,default=.02);p.add_argument('--implicit-damage',action='store_true')

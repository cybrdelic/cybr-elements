...
os.environ.setdefault('CUDA_VISIBLE_DEVICES','0' if os.environ.get('LAVA_MPM_LINEAR_BACKEND')=='cuda_dense' or os.environ.get('LAVA_MPM_FRICTION_BACKEND')=='cuda_batched' else '-1')
...
_SOURCE_MODULES=['lava_mpm.py','lava_mpm_coupled.py','lava_mpm_phase_field.py','lava_mpm_fracture.py','lava_mpm_friction.py','lava_mpm_transfer.py','lava_mpm_sparse_pressure.py','lava_mpm_linear27.py','lava_mpm_cuda_contact27.py','lava_mpm_active_contact33.py','lava_mpm_inlet.py']
SOURCE_HASHES={name:hashlib.sha256((Path(__file__).parent/name).read_bytes()).hexdigest() for name in _SOURCE_MODULES}
SQ2 = np.sqrt(2.)
...
class Material:
    ...
    emissivity: float = .94
    convection: float = 12.
    heat_integrator: str = 'backward_euler'
    ...
    rheology: str = 'creeping_network'
    creep_viscosity_reference: float = 1.e12
    creep_temperature: float = 933.15
    creep_activation_over_R: float = 60000.
    fracture_model: str = 'phase_field'
    fracture_length: float = .005
    melt_viscosity_law: str = 'arrhenius'
    vft_a: float = -4.55
    vft_b: float = 5978.4
    vft_c: float = 595.3
    rock_creep_a: float = 6.1e8
    rock_creep_n: float = 3.6
    rock_creep_q: float = 456000.
    ...
    def viscosity(self, t):
        if self.melt_viscosity_law=='farrell_180719':
            # Farrell (2020), equation 3; Pa s and kelvin. The VFT curve
            # models the dry experimental melt, not crystalline rock creep.
            t=np.asarray(t,dtype=float)
            if np.any(t<=self.vft_c):raise ValueError('Temperature outside VFT domain')
            return 10.**np.minimum(300.,self.vft_a+self.vft_b/(t-self.vft_c))
        if self.melt_viscosity_law!='arrhenius':raise ValueError('Unknown melt viscosity law')
    ...
    def network(self, t, dt, stress=None):
        """Crystal suspension and a thermally activated Maxwell skeleton.

        The suspension contribution vanishes at crystal packing, while the
        separate creep contribution remains finite. Coherence is therefore
        independent of the elastic-to-viscous relaxation time. Legacy caches
        retain the historical packing rule explicitly. Neither parameter set
        is claimed to be a composition-fitted crystallization model.
        """
        solid = self.solid(t)
        ...
        if self.rheology == 'creeping_network':
            # Packing creates a coherent skeleton, not an infinitely viscous
            # solid. A separate thermally activated creep branch remains.
            # These are nominal parameters, not a fit to a basalt sample.
            exponent=self.creep_activation_over_R*(1/self.creep_temperature-1/np.asarray(t))
            fluidity=fluidity+np.exp(np.clip(exponent,-700,700))/self.creep_viscosity_reference
        elif self.rheology == 'basalt_power_creep':
            # Violay et al. (2012), GFB power-law parameters, Table 4.
            # Their stress unit is MPa; the solver uses Pa. J2 equivalent
            # creep rate gives fluidity = 3*epsdot_eq/sigma_eq. This is a
            # confined, glass-free rock reference, not a fit at atmospheric
            # pressure or a claim that its composition matches the melt.
            sigma=np.zeros_like(t,dtype=float) if stress is None else np.maximum(np.asarray(stress),0.)
            fluidity+=3*self.rock_creep_a/1e6*(sigma/1e6)**(self.rock_creep_n-1)*np.exp(-self.rock_creep_q/(8.314462618*np.asarray(t)))
        elif self.rheology != 'legacy_packing':
            raise ValueError('Unknown crystal-network rheology')
...
def heat_solve(material, h0, mass, K, area, gas_temperature, dt, bed_area=None, bed_conductance=0., bed_temperature=293.15,conduit_conductance=0.,conduit_temperature=293.15,initial_temperature=None):
    """Nonlinear backward Euler enthalpy solve. All boundary losses metered."""
    t0=material.temperature(h0) if initial_temperature is None else np.asarray(initial_temperature); t=t0.copy(); ba=np.zeros_like(area) if bed_area is None else bed_area
    ...
    for it in range(35):
        ...
        conduit=conduit_conductance*(t-conduit_temperature)
        residual=mass*(material.enthalpy(t)-h0)+dt*(K@t+radiation+convection+bed+conduit)
        norm=float(np.linalg.norm(residual)); residual0=norm if residual0 is None else residual0
        if norm <= 1e-9*max(residual0,1): break
        diagonal=mass*material.capacity(t)+dt*(4*material.emissivity*sigma*area*t**3+material.convection*area+bed_conductance*ba+conduit_conductance)
        ...
        for _ in range(14):
            trial=t+step*delta
            rr=mass*(material.enthalpy(trial)-h0)+dt*(K@trial+material.emissivity*sigma*area*(trial**4-material.ambient**4)+material.convection*area*(trial-gas_temperature)+bed_conductance*ba*(trial-bed_temperature)+conduit_conductance*(trial-conduit_temperature))
    ...
    conduit=conduit_conductance*(t-conduit_temperature)*dt
    balance=float(np.dot(mass,hn-h0)+np.sum(radiation+convection+bed+conduit))
    return hn,dict(radiation=radiation,convection=convection,bed=bed,conduit=conduit,balance=balance,iterations=it+1)


def bounded_heat_transfer(old,grid_old,grid_new,local,weights,mass):
    """Conservative PIC/FLIP blend satisfying a discrete maximum principle.

    Both candidates conserve enthalpy. A common blend preserves that equality;
    independently clipping particles would not. Bounds include the old
    material and solved nodal values, allowing legitimate external heating.
    At zero time the transfer is identity.
    """
    flip=old+np.sum(weights*(grid_new-grid_old)[local],axis=1)
    pic=np.sum(weights*grid_new[local],axis=1)
    lo=min(float(old.min()),float(grid_new.min()));hi=max(float(old.max()),float(grid_new.max()))
    delta=flip-pic;alpha=1.;positive=delta>0;negative=delta<0
    if positive.any():alpha=min(alpha,float(np.min((hi-pic[positive])/delta[positive])))
    if negative.any():alpha=min(alpha,float(np.min((lo-pic[negative])/delta[negative])))
    alpha=float(np.clip(alpha,0,1));result=pic+alpha*delta
    energy_error=float(mass@(result-flip))
    if abs(energy_error)>1e-9*max(float(np.sum(mass*abs(old))),1.):raise RuntimeError(('Bounded heat transfer lost energy',energy_error))
    return result,dict(flipFraction=alpha,unlimitedMaximumOvershootJPerKg=float(max(0.,flip.max()-hi,lo-flip.min())),transferEnergyErrorJ=energy_error)
...
def ground_map(xyz,dx,labels=None,ground_mask=None,cell_size=None):
    ...
    if len(lookup)!=len(xyz):raise ValueError('Ground grid-coordinate keys are not unique')
    rr=list(free);cc=list(range(len(free)));dd=[1.]*len(free)
    for i in np.flatnonzero((xyz[:,2]<-hz*1e-7)&grounded):
        q=xyz[i].copy();q[2]*=-1;j=lookup.get((*np.round((q-anchor)/cell).astype(int),int(labels[i])))
...
class MPM:
    def __init__(self, x, spacing, dx, temperature=1450., material=None, origin=None, shape=None, gravity=(0,0,-9.81), ground=True,cell_size=None,sample_size=None,contact_solver='friction',bonded_bed=False,coherent_fraction=None,failure_fraction=None):
        ...
        self.transfer='apic';self.transfer_filter_seconds=.001
    ...
    def step(self,dt,thermal=True,mechanics=True,gas=None,bed=None,boundary=None,node_velocity=None):
        if mechanics and self.material.fracture_model == 'phase_field':
            from lava_mpm_coupled import coupled_step
            return coupled_step(self,dt,thermal=thermal,mechanics=mechanics,gas=gas,bed=bed,boundary=boundary,node_velocity=node_velocity)
        return self._step(dt,thermal=thermal,mechanics=mechanics,gas=gas,bed=bed,boundary=boundary,node_velocity=node_velocity)

    def _step(self,dt,thermal=True,mechanics=True,gas=None,bed=None,boundary=None,node_velocity=None):
        ...
        conduit_conductance=0.;conduit_temperature=m.ambient
        if boundary is not None and 'wall_temperature' in boundary:
            from lava_mpm_inlet import thermal_boundary
            exposed,wall_area=thermal_boundary(xyz,exposed,cell_size,boundary)
            bottom,pipe_bottom=thermal_boundary(xyz,bottom,cell_size,boundary)
            wall_area+=pipe_bottom
            conduit_conductance=wall_area*m.conductivity/(.5*float(np.min(cell_size)))
            conduit_temperature=boundary['wall_temperature']
        ...
        if thermal:
            ...
            integrate_heat=heat_solve
            if m.heat_integrator=='trbdf2':
                from lava_mpm_heat38 import heat_solve_trbdf2 as integrate_heat
            elif m.heat_integrator!='backward_euler':raise ValueError('Unknown heat integrator')
            hn,heat=integrate_heat(m,ha,safe,K,exposed,tg,dt,bottom,bed_conductance,bed_temperature,conduit_conductance,conduit_temperature)
            self.h,heat_transfer=bounded_heat_transfer(self.h,ha,hn,local,w,self.mass)
            for key in ('radiation','convection','bed'):self.ledger[key]+=float(heat[key].sum())
            self.ledger['conduit_heat']=self.ledger.get('conduit_heat',0.)+float(np.sum(heat['conduit']))
        ...
        self.__dict__.pop('_friction_report',None)
        if mechanics:
            # Topological release uses the last accepted damage. Letting a
            # nonlinear trial tear/re-weld its grid stencil at d=0.995 made
            # the fixed-point map discontinuous and stalled crack propagation.
            # Damage still enters the implicit constitutive stiffness here;
            # newly accepted failures release on the following increment.
            topology_damage=getattr(self,'_accepted_damage',self.damage) if m.fracture_model=='phase_field' else self.damage
            labels=self.connectivity.update(self.x,solid,topology_damage,self.principal_direction,self.sample_size,self.coherent_fraction)
            ...
            shear,network_fluidity,relax,network_viscosity=m.network(temperature,dt,getattr(self,'_creep_stress_guess',None))
            ...
            phase_field=m.fracture_model=='phase_field'
            surviving=np.maximum((1-self.damage)**(2 if phase_field else 1),1e-8)
            ...
            bulk_relax=np.ones(n)
            if phase_field:
                # Positive volume strain belongs to the creeping skeleton;
                # negative volume strain retains incompressible melt support.
                bulk_relax=1/(1+dt*m.liquid_bulk*network_fluidity/3)
                # Dislocation creep is isochoric. Its shear fluidity must
                # not also relax the bulk elastic strain.
                if m.rheology=='basalt_power_creep':bulk_relax=np.ones(n)
                # Positive/negative split: baseline tensile skeleton plus a
                # unilateral compression pressure solved as a dual variable.
                bulk=m.liquid_bulk*np.where(can_tension,surviving*bulk_relax,0.)
            p0=bulk*strain0
            ...
            tangentC=csr_matrix((0,len(grid_ids)*3));tangent_impulse=np.zeros(0)
            pressureC=csr_matrix((0,len(grid_ids)*3));pressure_bound=np.zeros(0);pressure_compliance=np.zeros(0)
            if phase_field:
                extra=np.asarray(W@(m.liquid_bulk-bulk)).ravel();pressure_selected=extra>m.liquid_bulk*1e-10
                pressureC=(-dt*P)[pressure_selected];pressure_bound=np.asarray(W@strain0).ravel()[pressure_selected]
                pressure_compliance=1/(cv[pressure_selected]*extra[pressure_selected]);has_contact|=bool(pressureC.shape[0])
            if has_contact:
                ...
                contactC=vstack([fragmentC,wallC,pressureC],format='csr')
                physical_rows=fragmentC.shape[0]+wallC.shape[0]
                compliance=np.r_[np.full(physical_rows,1/(m.young*np.cbrt(cell_volume)*dt*dt)),pressure_compliance]
                offset=contactC@lift-np.r_[np.zeros(fragmentC.shape[0]),wall_bound,pressure_bound]
                contact_solve=implicit_contact
                contact_options={}
                if contactC.shape[0]>=96:
                    from lava_mpm_sparse_pressure import contact as contact_solve
                    contact_options['deadline']=getattr(self,'_solve_deadline',None)
                if self.contact_solver=='admm':
                    from lava_mpm_contact_admm import contact as contact_solve
                    contact_options={}
                mu=np.zeros(contactC.shape[0])
                if len(pairs):mu[:len(pairs)]=m.friction*((phase_mass[pairs]<ma[pairs]*.5).all(1))
                mu[len(pairs)+pipe_rows:physical_rows]=m.friction
                if self.contact_solver=='friction' and mu.any():
                    from lava_mpm_friction import contact as friction_contact,tangent_rows
                    friction_kwargs=dict(deadline=getattr(self,'_solve_deadline',None))
                    if pressureC.shape[0]>=32:
                        from lava_mpm_friction import mixed_contact as friction_contact
                        friction_kwargs['physical_rows']=physical_rows
                    u,mechanical_residual,multipliers,contact_objective_loss,contact_iterations,tangentC,tangent_impulse,friction_report=friction_contact(A,rhs,S,contactC,compliance,offset,mu,tangent_rows(contactC,mu>0)@lift,**friction_kwargs)
                    self._friction_report=friction_report
                    self.ledger['friction_work']=self.ledger.get('friction_work',0.)+friction_report['slidingDissipationJ']
                else:
                    u,mechanical_residual,multipliers,contact_objective_loss,contact_iterations=contact_solve(A,rhs,S,contactC,compliance,offset,**contact_options)
                contact_count=int((multipliers[:physical_rows]>0).sum())
                impulse=np.asarray(fragmentC.T@multipliers[:fragmentC.shape[0]]).reshape(-1,3)
                if tangentC.shape[0]:impulse+=np.asarray(tangentC[:2*fragmentC.shape[0]].T@tangent_impulse[:2*fragmentC.shape[0]]).reshape(-1,3)
                contact_error=float(np.linalg.norm(impulse.sum(0)))
                ...
                floor_impulse=np.asarray(wallC[pipe_rows:].T@multipliers[fragmentC.shape[0]+pipe_rows:physical_rows]).reshape(-1,3).sum(0)
            ...
            if boundary is not None or node_velocity is not None:
                ...
                if has_contact:reaction+=contactC.T@multipliers+tangentC.T@tangent_impulse
            ...
            self._trial_creep_stress=np.sqrt(1.5*np.sum((elasticdev/surviving[:,None])**2,axis=1))
            newdev=elasticdev+2*solvent_viscosity[:,None]*rate
            dilation_rate=np.asarray(P@un.ravel()).ravel()[ci]
            trial_dilation=strain0+dt*dilation_rate
            self._trial_dilation=trial_dilation.copy()
            newdilation=trial_dilation.copy()
            if phase_field:
                compression=trial_dilation<0
                newdilation=np.where(compression,newdilation,newdilation*bulk_relax)
                bulk=m.liquid_bulk*np.where(compression,1.,np.where(can_tension,surviving*bulk_relax,0.))
            newdilation=np.where(can_tension,newdilation,np.minimum(newdilation,0))
            fullstress=tensor(newdev)+np.eye(3)[None]*(bulk*trial_dilation)[:,None,None]
            eig,vectors=np.linalg.eigh(fullstress)
            # Vanishing post-failure stress has no reliable principal axis.
            # Preserve the normal at failure, then advect its orientation
            # with the material rotation below.
            self.principal_direction=np.where((self.damage>.995)[:,None],self.principal_direction,vectors[:,:,-1])
            eig=eig[:,-1]/surviving
            fracture_length=1/np.sqrt(np.sum((self.principal_direction/cell_size)**2,axis=1))
            if phase_field:
                # Trial tensile elastic energy belongs to the undegraded
                # network. Newtonian liquid stress must not drive fracture.
                from lava_mpm_phase_field import update_damage
                undamaged_dev=elasticdev/surviving[:,None]
                tensile_energy=np.sum(undamaged_dev**2,axis=1)/(4*np.maximum(shear,1.))+.5*m.liquid_bulk*np.maximum(newdilation,0)**2
                eligible=solid>self.failure_fraction
                damage,phase_report=update_damage(self.rest,self.mass/m.density,self.sample_size,getattr(self,'_accepted_damage',self.damage),tensile_energy,eligible,m.fracture_energy,m.fracture_length)
                self._phase_report=phase_report
            ...
            # Use the same residual stiffness in the trial and final states.
            # An Anderson trial may hit damage=1 exactly. Dividing by an
            # unfloored (1-d)^2 then amplified stress and dissipative heat
            # when the converged phase field returned slightly below one.
            final_surviving=np.maximum((1-damage)**(2 if phase_field else 1),1e-8)
            damage_ratio=final_surviving/surviving
            elasticdev*=damage_ratio[:,None]
            # Maxwell dissipation is converted into enthalpy, not discarded.
            dissipation=dt*self.volume*(np.sum(elasticdev*elasticdev,axis=1)*network_fluidity/(2*final_surviving)+2*solvent_viscosity*np.sum(rate*rate,axis=1))
            if phase_field and m.rheology!='basalt_power_creep':
                dissipation+=dt*self.volume*final_surviving*(m.liquid_bulk*np.maximum(newdilation,0))**2*network_fluidity/3
            if tangentC.shape[0]:
                # Work of Coulomb traction against the converged slip is
                # heat. Distribute each interface's heat to its supported
                # material nodes, then conservatively back to particles.
                slip=np.asarray(tangentC@u).reshape(-1,2)
                pair_heat=np.maximum(0.,np.sum(tangent_impulse.reshape(-1,2)*slip,axis=1))
                grid_heat=np.zeros(len(ma))
                for k,heat in enumerate(pair_heat):
                    rowC=contactC.getrow(k);nodesC=rowC.indices//3
                    weights=np.abs(rowC.data);weights/=max(weights.sum(),1e-30)
                    np.add.at(grid_heat,nodesC,heat*weights)
                particle_heat=self.mass*np.sum(w*(grid_heat/safe)[local],axis=1)
                dissipation+=particle_heat
                self.ledger['friction_heat']=self.ledger.get('friction_heat',0.)+float(particle_heat.sum())
            ...
            if self.transfer=='impulse_affine':
                from lava_mpm_transfer import transfer
                newv,newC=transfer(self.v,self.C,gv,un,local,w,dp,cell_size,dt,self.transfer_filter_seconds)
            elif self.transfer!='apic':raise ValueError('Unknown particle/grid transfer')
            ...
            self.principal_direction=np.einsum('pij,pj->pi',rotation,self.principal_direction)
            ...
            if boundary is not None and 'pipe_end' in boundary:
                from lava_mpm_inlet import conduit_gap
                if np.min(conduit_gap(self.x,boundary))<-.25*float(np.min(cell_size)):
                    raise RuntimeError('Trial penetrated conduit; reject before checkpointing')
        ...
        balance=total-self.initial_energy-self.ledger.get('source_enthalpy',0.)+self.ledger['radiation']+self.ledger['convection']+self.ledger['bed']+self.ledger.get('conduit_heat',0.)-self.ledger['viscous_heat']-self.ledger.get('contact_heat',0.)+self.ledger.get('dust_enthalpy_out',0.)
        ...
        broken_solid=self.connectivity.broken & self.connectivity.frozen[self.connectivity.edges].all(1)
        row['fracturedSolidEdges']=int(broken_solid.sum())
        row['meltReleasedEdges']=int((self.connectivity.broken & ~broken_solid).sum())
        row['massBalanceErrorKg']=mass_error
        if thermal:row['heatTransfer']=heat_transfer
        if hasattr(self,'_friction_report'):row['frictionContact']=self._friction_report
    ...
    def save(self,folder):
        ...
        path=folder/'state.npz';temp=folder/'state.partial.npz';np.savez_compressed(temp,**state)
        ...
        meta['sourceHashes']=dict(SOURCE_HASHES)
        meta['pressureSolver']=dict(lineSearch=os.environ.get('LAVA_MPM_PRESSURE_LINESEARCH','exact'),newtonSystem=os.environ.get('LAVA_MPM_PRESSURE_NEWTON','primal'))
        from lava_mpm_linear27 import metrics
        meta['linearBackend']=metrics()
        from lava_mpm_cuda_contact27 import metrics as contact_metrics
        meta['frictionBackend']=contact_metrics()
        meta.update(transfer=self.transfer,transfer_filter_seconds=self.transfer_filter_seconds)
        if hasattr(self,'driver_state'):meta['driver_state']=self.driver_state
        # Prepare both files before replacing either accepted file. A disk
        # failure during JSON writing must not pair new particles with an old
        # thermal ledger. Readers also verify the common time below.
        tmp=folder/'state.partial.json';tmp.write_text(json.dumps(meta,indent=2))
        temp.replace(path);tmp.replace(folder/'state.json')

    @classmethod
    def load(cls,folder):
        folder=Path(folder);meta=json.loads((folder/'state.json').read_text());s=np.load(folder/'state.npz')
        if 'time' in s and abs(float(s['time'])-float(meta['time']))>1e-10:
            raise ValueError('Checkpoint data and metadata are from different steps')
        material=dict(meta['material'])
        # Historical caches must retain their actual constitutive equations.
        material.setdefault('rheology','legacy_packing')
        material.setdefault('fracture_model','crack_band')
        obj=cls(s['x'],meta['spacing'],meta['dx'],material=Material(**material),origin=meta['origin'],shape=meta['shape'],gravity=meta['gravity'],ground=meta['ground'],cell_size=meta.get('cell_size'),sample_size=meta.get('sample_size'),contact_solver=meta.get('contact_solver','dense'),bonded_bed=meta.get('bonded_bed',False),coherent_fraction=meta.get('coherent_fraction',.85),failure_fraction=meta.get('failure_fraction',.8))
        ...
        obj.transfer=meta.get('transfer','apic');obj.transfer_filter_seconds=meta.get('transfer_filter_seconds',.001)
        if 'driver_state' in meta:obj.driver_state=meta['driver_state']
        return obj

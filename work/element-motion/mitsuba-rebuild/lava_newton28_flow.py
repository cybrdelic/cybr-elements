...
def run(name,pitch=.0015,until=8.,wall=120.,max_dt=.02,from_cache=None):
    folder=ROOT/name;folder.mkdir(parents=True,exist_ok=True);wp.init()
    with wp.ScopedDevice('cuda:0'):
        source=hashes();resume=(folder/'state.npz').exists();migrated=False
        if resume:
            ...
            saved=dict(np.load(from_cache));rest=saved['rest'];x=saved['x'];v=saved['v'];clock=float(saved['time']);rows=[]
            if float(saved['pitch'])!=pitch:raise ValueError('Migration cannot change particle spacing')
            ledger=dict(boundaryLossJ=0.,pairConductionResidualJ=0.,plasticHeatJ=0.,fractureWorkJ=0.);migrated=True
            meta=dict(parentCache=str(from_cache),parentSha256=hashlib.sha256(Path(from_cache).read_bytes()).hexdigest(),ledgerEpochSeconds=clock)
        else:
        ...
        strength=saved['strength'] if resume or migrated else np.clip(rng.normal(1.,.10,n),.7,1.3).astype('f4')
        strength=wp.array(strength,dtype=float)
        h=wp.array(saved['h'] if resume or migrated else np.full(n,732220.,dtype='f4'),dtype=float)
        hnew=wp.empty_like(h)
        damage=wp.array(saved['damage'] if resume or migrated else np.zeros(n,dtype='f4'),dtype=float)
        plastic=wp.array(saved['plastic'] if resume or migrated else np.zeros(n,dtype='f4'),dtype=float)
        if resume or migrated:
        ...
        wp.launch(update_material,n,inputs=[h,h,damage,strength,a.mpm.particle_elastic_strain,*params,0])
        solver=SolverImplicitMPM(model,SolverImplicitMPM.Config(voxel_size=voxel,tolerance=1e-7,max_iterations=500,air_drag=.001,grid_type='sparse'),verbose=False)
        ...
        def save(label):
            ...
            if not all(np.isfinite(v).all() for v in state.values()):
                np.savez_compressed(folder/'rejected-state-diagnostics.npz',**state)
                raise RuntimeError('Nonfinite state; physical cache rejected; diagnostic arrays preserved separately')
            np.savez_compressed(folder/f'{label}.npz',**state)
            return state

        if not resume:save(f'frame-{round(clock*1000):06d}')
        try:
            while clock<until-1e-8:
                ...
                wp.launch(update_material,n,inputs=[h,hnew,damage,strength,a.mpm.particle_elastic_strain,*params,1])
        ...
        state=save('state')
        receipt=dict(status='complete' if error is None else 'incomplete',error=error,time=clock,target=until,steps=steps,seconds=time.monotonic()-ticks,particles=n,pitchM=pitch,voxelM=voxel,initialEnergyJ=initial_energy,ledger=ledger,rows=rows,sourceHashes=source,newtonVersion=newton.__version__,warpVersion=wp.__version__,maxDtSeconds=max_dt,solverTolerance=1e-7,solverMaxIterations=500,parentCache=meta.get('parentCache') if resume or migrated else None,parentSha256=meta.get('parentSha256') if resume or migrated else None,ledgerEpochSeconds=meta.get('ledgerEpochSeconds',0.) if resume or migrated else 0.,
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--name',default='cooling-flow');p.add_argument('--pitch',type=float,default=.0015);p.add_argument('--until',type=float,default=8.);p.add_argument('--wall',type=float,default=120.);p.add_argument('--dt',type=float,default=.02);p.add_argument('--from-cache',type=Path);a=p.parse_args()
    r=run(a.name,a.pitch,a.until,a.wall,a.dt,a.from_cache);raise SystemExit(r['status']!='complete')

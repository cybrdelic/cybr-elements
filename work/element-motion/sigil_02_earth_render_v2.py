...
R=Path(__file__).resolve().parent;O=R/'sigil-02-elements';out=O/'earth-frames-v2';out.mkdir(exist_ok=True)
data=np.load(O/'earth-motion-v2.npz');poses=data['transforms'];radii=data['radii'];birth=data['births']
...
if '--cpu-pilot' in sys.argv:
 s.cycles.device='CPU';s.cycles.denoiser='OPENIMAGEDENOISE';s.render.resolution_x=1280;s.render.resolution_y=720;out=O/'earth-pilot-v2';out.mkdir(exist_ok=True)
...
bpy.ops.object.camera_add(location=(0,-17,2.8875));cam=bpy.context.object;cam.rotation_euler=(Vector((0,0,2.8875))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.type='ORTHO';cam.data.ortho_scale=14;s.camera=cam

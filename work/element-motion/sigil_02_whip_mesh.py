...
O=R/'sigil-02-water-whip'/('full' if '--full' in sys.argv else 'cpu');source=O/'particles';out=O/'mesh';out.mkdir(exist_ok=True);pilot='--pilot' in sys.argv
preview='--preview' in sys.argv
if preview:out=O/'preview-mesh';out.mkdir(exist_ok=True)
frames=list(range(0,180,6)) if preview else [0,18,36,54,72,90,114,150,174] if pilot else range(390);isolated=np.empty(0,bool);previous_ids=np.empty(0,np.uint32);previous_iso=1.8;begun=time.time()
...
for f in frames:
 ...
 if not pilot and f not in [0,36,66,90,120,180,270,389]:

...
O=R/'sigil-02-water-hold';source=O/'particles';out=O/'mesh';out.mkdir(exist_ok=True);pilot='--pilot' in sys.argv
preview='--preview' in sys.argv
if preview:out=O/'preview-mesh';out.mkdir(exist_ok=True)
frames=range(60,166,3) if preview else [45,75,120,165] if pilot else range(300);isolated=np.empty(0,bool);previous_ids=np.empty(0,np.uint32);previous_iso=1.8;begun=time.time()

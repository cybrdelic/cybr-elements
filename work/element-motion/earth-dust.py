...
def step(t):
    ...
    soot.lerp_(torch.ones_like(soot)*.38,inject);temp.zero_()
    v=state[0,:3]
...
def capture(frame,t,div):
    ...
    rgb=sigma[...,None]*depthLight[...,None]*torch.tensor([.48,.30,.15],device=device)*1.8
...
W,H=1920,1080
video=ROOT.parent.parent/'outputs/cybrdelic-type/elements/motion/earth-dust.mp4'

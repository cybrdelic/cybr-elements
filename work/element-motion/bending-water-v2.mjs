...
const out=new URL('bending-rebuild-v2/water-particles/',import.meta.url);
...
Object.assign(c,{nameKey:'bending-sheet',h:.018,nx:234,ny:156,nz:40,
 extent:[234*.018,156*.018,40*.018],obstacles:[],maxParticles:180000,seed:93157,
 gravity:[0,-1.7,0],flip:.94,separation:true,surfaceTension:.072});
function pose(t){
...
class RibbonFlow extends FlipSolver{
 emit(dt){
  ...
  const width=.092*(1+.12*Math.sin(t*13)),depth=.029;
  ...
  const roll=.52*Math.sin(t*7.7)+.23*Math.sin(t*14.3),co=Math.cos(roll),si=Math.sin(roll);
  for(let i=0;i<count;i++){
   ...
   const speed=1.65+.70*(1-rad*rad),omega=7.5+2.5*Math.sin(t*9);
   const normal=.33*aa/width-omega*back;
   const vz=omega*across+.10*Math.sin(t*23+aa*35);
...
const manifest={config:c,frameDt:timeScale/30,playbackFps:30,timeScale,spaceScale,origin,frames:[],source:'New elliptical sheet emitter; native FLIP/APIC pressure and capillary motion; authored force lift and local turning; floor below camera'};
...
for(let f=0;f<120;f++){
 ...
 if(f===45&&!process.argv.includes('--ungated')){
  console.log('PREVIEW GATE: primary states 0..45 ready; waiting for continue-water file');
  while(!fs.existsSync(new URL('bending-rebuild-v2/continue-water',import.meta.url)))await new Promise(resolve=>setTimeout(resolve,500));
 }
}

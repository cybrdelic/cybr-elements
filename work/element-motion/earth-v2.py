...
R=Path(__file__).resolve().parent;data=json.loads((R/'earth-geometry.json').read_text());out=R/'earth-v2-frames';out.mkdir(exist_ok=True)
...
for i,d in enumerate(data['pieces']):
 me=bpy.data.meshes.new('fracture');me.from_pydata(d['verts'],[],d['faces']);me.update();o=bpy.data.objects.new(f'Rock {i}',me);bpy.context.collection.objects.link(o);o.location=d['center'];o.data.materials.append(stone);bpy.context.view_layer.objects.active=o;o.select_set(True);bpy.ops.rigidbody.object_add();o.select_set(False);o.rigid_body.mass=max(.2,d['volume']*2700);o.rigid_body.friction=.72;o.rigid_body.restitution=.08;o.rigid_body.collision_shape='CONVEX_HULL';o.rigid_body.use_margin=True;o.rigid_body.collision_margin=.0015;pieces.append(o)
for a,b in data['links']:
 bpy.ops.object.empty_add(location=(pieces[a].location+pieces[b].location)*.5);o=bpy.context.object;bpy.ops.rigidbody.constraint_add();c=o.rigid_body_constraint;c.type='FIXED';c.object1=pieces[a];c.object2=pieces[b];c.use_breaking=True;c.breaking_threshold=350+400*abs(math.sin(a*2+b));c.disable_collisions=True
# A visible moving rock supplies the impact. Subsequent fracture and contact are Bullet dynamics.
bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=2,radius=.48,location=(-3.4,0,.66));hammer=bpy.context.object;hammer.name='Incoming rock';hammer.data.materials.append(stone);bpy.ops.rigidbody.object_add();hammer.rigid_body.mass=420;hammer.rigid_body.kinematic=True;hammer.keyframe_insert('location',frame=1);hammer.location=(-2.5,0,.66);hammer.keyframe_insert('location',frame=22);hammer.location=(1.7,0,.72);hammer.keyframe_insert('location',frame=48);hammer.rigid_body.keyframe_insert('kinematic',frame=48);hammer.rigid_body.kinematic=False;hammer.rigid_body.keyframe_insert('kinematic',frame=49)
...
bpy.ops.object.camera_add(location=(4.8,-8,4.3));cam=bpy.context.object;cam.rotation_euler=(Vector((.15,0,.5))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.lens=47;s.camera=cam;s.view_settings.view_transform='AgX';start=time.time()

...
out = ROOT/'flow-frames'
out.mkdir(exist_ok=True)
if (out/'complete.json').exists() and not a.resume: raise RuntimeError('Completed bake exists; use a new name')
if shutil.disk_usage(out).free < 1024**3: raise RuntimeError('1 GiB MP4 free-space reserve required')
...
lo=np.array([-5.25,-.50,0],np.float32)
extent=np.array([10.5,1.,4.2],np.float32)
...
def step(t):
    ...
    # Only point-like nozzles inject fuel, briefly. No glyph-shaped source.
    v=state[0,:3]
    ...
    # Low-frequency momentum guidance; no scalar field is fitted or moved.
...
import sys
FPS=30
DURATION=float(os.environ.get('INTRO_SECONDS','6.5'))
TOTAL=round(DURATION*FPS)
W,H=1920,1080
video=ROOT.parent/'outputs'/'cybrdelic-flow-intro.mp4'
...
(ROOT/'flow-report.json').write_text(json.dumps({'frames':TOTAL,'fps':FPS,'size':[W,H],'solverGrid':a.size,'elapsed':time.monotonic()-started,'peakGpuMiB':torch.cuda.max_memory_allocated()/1048576,'source':'bake-reactive-fire.py','changes':'Short point nozzles; momentum guided stroke flow; native reactive advection and pressure; finite fuel pulses followed by release.'},indent=2))

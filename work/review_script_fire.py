...
v=root.parent/'outputs'/'cybrdelic-script-fire.mp4'
...
contact([1.5,3.,4.5,6.,8.,10.7,11.7,13.],'script-fire-review.jpg')
contact([round(10.8+i*.2,2) for i in range(12)],'script-burnout-review.jpg')
...
source=(root/'render_script_fire.py').read_text()
assert 'closing=' not in source and '*closing' not in source
rows=[]
for line in (root/'script-fire.log').read_text().splitlines():
...
(root/'script-fire-verification.json').write_text(json.dumps(report,indent=2))
print(json.dumps({'metadata':s,'decode':'all frames passed',**ratios,'remainingGasRatio':rows[-1]['gasInventory']/rows[0]['gasInventory'],'review':['script-fire-review.jpg','script-burnout-review.jpg']}))

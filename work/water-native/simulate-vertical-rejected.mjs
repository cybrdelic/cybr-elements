...
const c=makeProductionPreset('jets');Object.assign(c,{nameKey:'sigil',h:.035,nx:160,ny:100,nz:60,extent:[5.6,3.5,2.1],obstacles:[],maxParticles:300000,seed:8460+(+variant),gravity:[0,-9.81,0]});
class Flow extends FlipSolver{
 emit(dt){
  if(this.time>.60)return;
  const counts=new Map(),h=this.h,key=(x,y,z)=>Math.floor(x/h)+160*(Math.floor(y/h)+100*Math.floor(z/h));
  for(let i=0;i<this.count;i++){const q=i*3;if(Math.abs(this.p[q+2]-.86)>.09)continue;const k=key(...this.p.subarray(q,q+3));counts.set(k,(counts.get(k)||0)+1);}
  const front=.8+4*Math.min(1,(this.time+dt)/.22),fade=Math.max(0,Math.min(1,(.60-this.time)/.08));
  for(const p of source.points){if(p[0]>front||this.random()>fade)continue;const k=key(...p);if((counts.get(k)||0)>=7)continue;
   ...
   const vx=.12*Math.sin(y*8),vy=.45+.12*Math.sin(x*5),vz=.65+.14*Math.sin(x*7+y*8);

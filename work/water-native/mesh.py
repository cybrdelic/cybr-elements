...
variant=sys.argv[1];folder=R/f'cache-{variant}';m=json.loads((folder/'manifest.json').read_text())
out=R.parent.parent/f'outputs/cybrdelic-type/elements/water/cache/{variant}';out.mkdir(parents=True,exist_ok=True)
all_frames=len(sys.argv)>2 and sys.argv[2]=='all'
frames=range(192) if all_frames else [int(x) for x in sys.argv[2].split(',')] if len(sys.argv)>2 else range(len(m['frames']));stats=[];start=time.time()

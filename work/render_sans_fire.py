...
if a.gesture_version=='23':
    ...
    gesture_pose=None
from sans_motion import pose as nozzle_pose, turn_rate as nozzle_turn, WRITE, START, MODE
if not a.name.replace('-', '').isalnum(): raise ValueError('Expected a local artifact name')
out = ROOT/'sans-fire-frames'
...
W,H=1920,1080
video=ROOT.parent/'outputs'/'cybrdelic-sans-fire.mp4'
...
for frame in range(TOTAL):
    ...
    if frame in [239,329,TOTAL-1]:
        torch.save({'state':state,'reservoir':reservoir,'next_frame':frame+1,'size':a.size,'sim_fps':SIM_FPS,'substeps':a.substeps},ROOT/f'sans-fire-checkpoint-{frame+1}.pt')

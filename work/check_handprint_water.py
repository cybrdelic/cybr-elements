...
import handprint_motion as font
p=Path(__file__).with_name('render_water_handprint.py').read_text()
...
Path(__file__).with_name('water-handprint-dynamics-check.json').write_text(json.dumps(rows,indent=2))
print(rows)

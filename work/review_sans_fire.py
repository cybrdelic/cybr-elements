...
v=root.parent/'outputs'/'cybrdelic-sans-fire.mp4'
...
contact([1.5,3.,4.5,6.,8.,10.7,11.7,13.],'sans-fire-review.jpg')
contact([round(10.8+i*.2,2) for i in range(12)],'sans-burnout-review.jpg')
...
source=(root/'render_sans_fire.py').read_text()
assert 'closing=' not in source and '*closing' not in source
rows=[]
for line in (root/'sans-fire.log').read_text().splitlines():
...
(root/'sans-fire-verification.json').write_text(json.dumps(report,indent=2))
print(json.dumps({'metadata':s,'decode':'all frames passed',**ratios,'remainingGasRatio':rows[-1]['gasInventory']/rows[0]['gasInventory'],'review':['sans-fire-review.jpg','sans-burnout-review.jpg']}))

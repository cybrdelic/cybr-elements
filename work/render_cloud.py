...
"""
import os, json, time, math, subprocess, gzip
...
device=os.environ.get('FIRE_DEVICE','cuda');FPS=30;SECONDS=float(os.environ.get('MASS_SECONDS','6.5'))
...
def advance(u,delta):
    ...
    for step in range(n):
        ...
        if now<1.1:
            ...
            q=pos
            age=max(0.,now-T)
            curl=torch.stack([torch.sin(q[:,1]*5+now*3+phase*.1),torch.cos(q[:,0]*4-now*2+phase*.1),torch.sin(q[:,0]*3+q[:,1]*4-now*3)],dim=1)
            vel.add_((curl*(.22+min(age,1.5)*.4)+tensor([1.65,1.05,0.])*min(age*1.8,1.))*dt)
        pos.add_(vel*dt)

def image(u,frame):
    col=data['color']*(.85+.15*torch.sin(phase+u*4))[:,None]
    ignition=min(1.,max(0.,u/.20))
    cooling=math.exp(-max(0.,u-2.9)*.9)
...
(ROOT/'mass-report.json').write_text(json.dumps({'frames':round(SECONDS*FPS),'fps':FPS,'dimensions':[W,H],'parcels':len(pos),'source':'Spatial emission samples from native flame-native-emitter 3D cache','method':'3D cached-fire emission parcels; finite arrival force followed by free turbulent release; not coupled CFD','checks':checks},indent=2))

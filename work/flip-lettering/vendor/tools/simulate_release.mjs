/** Public III.1 generator using the same locked profile as the standalone CPU worker. */
import {FlipSolver} from '../src/flip.js';
import {makePreset,PRODUCTION_QUALITY} from '../src/quality-profile.js';
import {encodeCheckpoint} from '../src/checkpoint.js';
import fs from 'node:fs';import crypto from 'node:crypto';
const [name='impact',framesText='120',requestedQuality,outputName=name]=process.argv.slice(2),frames=Number(framesText);
const quality=requestedQuality??PRODUCTION_QUALITY[name],variant='repair';
if(!['live','test','high','ultra'].includes(quality))throw Error('Choose a supported scene and quality (live, test, high, ultra).');
...
const config=makePreset(name,quality);
config.repairVersion='3.1.0';config.variant=variant;
const solver=new FlipSolver(config),out=new URL(`../cache/${outputName}/`,import.meta.url);
fs.mkdirSync(out,{recursive:true});
const sha=b=>crypto.createHash('sha256').update(b).digest('hex');
const sourceFiles=['src/flip.js','src/flip-reference.js','src/flip-base.js','src/numerics/multigrid.js','src/numerics/stress.js','src/quality-profile.js','tools/simulate_release.mjs'];
const sourceHashes=Object.fromEntries(sourceFiles.map(p=>[p,sha(fs.readFileSync(new URL('../'+p,import.meta.url)))]));
const manifest={schema:'cybr-flip-cache/3',name:outputName,preset:name,config,frameDt:1/48,frames:[],sourceHashes,solver:'src/flip.js',
 rawFormat:'little-endian float32: positions xyz then velocities xyz; .shape is per-particle xx yy zz xy xz yz',
 simulationBackend:'CPU III.1 quadratic APIC/FLIP with Galerkin MG-PCG',createdAt:new Date().toISOString()};

/*
 * CYBR FLIP II — sparse WebGL2 compute backend.
 *
...
class SparseGPUFLIP{
 constructor(threeRenderer,config,positions,velocities,options={}){
  ...
  this.programs={};this.targets=new Set();this.textures=new Set();this.passCount=0;this.lastPressure=null;this.lastMetrics=null;
 ...
 read(target,index=0,out=null){const gl=this.gl;gl.bindFramebuffer(gl.FRAMEBUFFER,target.fbo);gl.readBuffer(gl.COLOR_ATTACHMENT0+index);const a=out??new Float32Array(target.width*target.height*4);gl.readPixels(0,0,target.width,target.height,gl.RGBA,gl.FLOAT,a);return a;}
 ...
 reduce(input,destination){let texture=input,w=input.width,h=input.height;for(const target of this.reduction){this.draw('reduce',target,{uInput:texture,uReduceSize:[w,h]});texture=target.textures[0];w=target.width;h=target.height;}this.draw('copy',destination,{uInput:texture});}
 ...
 async advance(frameDt=1/48){if(!(frameDt>0&&frameDt<=.25))throw Error('Invalid frame duration.');let remaining=frameDt,substeps=0;const gravity=Math.hypot(...(this.config.gravity??[0,-9.81,0])),sigma=this.config.surfaceTension??0,cap=sigma>0?.4*Math.sqrt((this.config.density??1000)*this.h**3/(Math.PI*sigma)):Infinity;while(remaining>1e-10){const dt=Math.min(remaining,1/90,.45*this.h/Math.max(.1,this.maxSpeed+gravity*remaining),cap);this.step(dt);remaining-=dt;const state=this.readState();if(!state.metrics.finite||state.metrics.solidViolations)throw Error('GPU particle validation failed.');if(++substeps>256)throw Error('GPU CFL budget exceeded.');}return {...this.lastMetrics,substeps};}

...
vec3 opticalBranch(vec3 origin,vec3 direction,ivec3 medium,out float lengthInWater){
 vec3 origins[16],directions[16],weights[16];ivec3 media[16];int depths[16];float glasses[16];int sp=1;
 origins[0]=origin;directions[0]=direction;weights[0]=vec3(1.);media[0]=medium;depths[0]=0;glasses[0]=1.5;
 vec3 result=vec3(0.);lengthInWater=0.;
 for(int iteration=0;iteration<28;iteration++){
  if(sp==0)break;sp--;vec3 ro=origins[sp],rd=directions[sp],weight=weights[sp];ivec3 layers=media[sp];int depth=depths[sp];float glass=glasses[sp];Hit hit;if(max(weight.r,max(weight.g,weight.b))<.008){result+=weight*environment(rd);continue;}
  if(!trace(ro,rd,hit)){result+=weight*environment(rd);continue;}
  ...
  if(hit.material!=0&&hit.material!=2&&glassIOR<1.){result+=weight*opaque(hit,point,rd);continue;}
  ...
  float ei=ior(layers,glass),et=ior(next,glass),F=dielectric(clamp(-dot(n,rd),0.,1.),ei,et);vec3 transmitted=refract(rd,n,ei/et),reflected=reflect(rd,n);
  if(dot(transmitted,transmitted)<.01)F=1.;
  if(depth>=7){result+=weight*(F*environment(reflected)+(1.-F)*environment(transmitted));continue;}
  if(F>.002&&sp<15){origins[sp]=point+reflected*uEpsilon*5.;directions[sp]=reflected;weights[sp]=weight*F;media[sp]=layers;depths[sp]=depth+1;glasses[sp]=glass;sp++;}
  if(F<.998&&sp<15){origins[sp]=point+transmitted*uEpsilon*5.;directions[sp]=normalize(transmitted);weights[sp]=weight*(1.-F);media[sp]=next;depths[sp]=depth+1;glasses[sp]=glass;sp++;}
 }
 return result;
}
void main(){
...
class GeometryOptics{
 ...
 build(){const l=this.liquid;if(this.builtFor===l.latest)return this.stats;const stats=this.reference.build(),u=this.reference.uniforms,r=l.material.uniforms;
  ...
  this.builtFor=l.latest;this.stats={primitiveCount:stats.primitiveCount,nodeCount:stats.nodeCount,buildMilliseconds:stats.buildMilliseconds,optics:'actual BVH dielectric paths; bounded branching; opaque irradiance approximate'};return this.stats;
 }

...
export function cacheDigest(value){const text=canonical(value);let h=14695981039346656037n;for(const b of new TextEncoder().encode(text)){h^=BigInt(b);h=BigInt.asUintN(64,h*1099511628211n);}return h.toString(16).padStart(16,'0');}
...
export function createSceneGraph(config){return new SceneGraph({schema:'cybr-scene-graph/1',units:{length:'m',time:'s',mass:'kg',surfaceTension:'N/m',kinematicViscosity:'m²/s'},nodes:[
...
export function planMemory(config,{width=1920,height=1080,particleCapacity=config.maxParticles??800000,activeBricks=null,surfaceTriangles=500000,budgetBytes=2**30}={}){

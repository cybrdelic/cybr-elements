import {FlipSolver,makeProductionPreset} from './vendor/src/main.js';
...
const config=makeProductionPreset('jets');Object.assign(config,{nameKey:'cybrdelic',h:.03,nx:156,ny:100,nz:36,extent:[4.68,3,1.08],obstacles:[],maxParticles:350000,seed:8107});
...
class Lettering extends FlipSolver{
 ...
 for(let j=0;j<count;j++){let u=this.random(),t=this.time+(end-this.time)*u,p=point(t),side=j%2?1:-1,r=(this.random()-.5)*.10,z=side*.042+(this.random()-.5)*.022;let vx=dx/Math.max(len,1e-8)*.20,vy=9.81*(.60-t)*.5+dy/Math.max(len,1e-8)*.20;
 if(this.add(p[0]+nx*r,p[1]+ny*r,p[2]+z,vx,vy,-side*1.3))this.spawned++;}
 }
}
...
for(let frame=0;frame<96;frame++){
 const info=sim.advance(1/96),n=sim.count;if(!info.finite||info.capacityRejected||!info.pressure.converged)throw Error(JSON.stringify(info));
 ...
 manifest.frames.push({frame,...info,primarySha256:sha(raw)});fs.writeFileSync(new URL('manifest.json',out),JSON.stringify(manifest));if(frame%4===0)console.log(JSON.stringify({frame,n,seconds:(performance.now()-start)/1000}),flush=>{});
}

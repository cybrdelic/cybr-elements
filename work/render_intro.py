...
The unmodified state sequence and native velocity are retained for verification.
"""
...
p.add_argument('--gesture-version', choices=['19','20','20b','21','21b','22','22b'], default='19')
...
if shutil.disk_usage(out).free < 1024**3: raise RuntimeError('1 GiB reserve required for streamed MP4; no volume caches written')
...
lo=np.array([-5.25,-.5,0],np.float32)
extent=np.array([10.5,1.,3.94],np.float32)
...
def step(t):
    ...
    inlet=1. if not a.pulse else (1. if .15<t<a.pulse_duration else 0.)
    # Source perturbations act on incoming gas, not on the finished fire shape.
    modulation=1+.18*torch.sin(x*28+y*19+t*9)+.10*torch.sin(y*43-t*13)
    if a.control=='choreographed':
        authored=gesture_pose(a.move,t)
        inlet*=authored.get('emission',1.)
        cx,cy,cz=authored['position'];dx,dy,dz=authored['direction']
        # Orthonormal nozzle frame. Keep the old inlet's section dimensions;
        # rotate incoming gas, not any already-emitted volume.
        tangent=np.cross([dx,dy,dz],[0.,1.,0.])
        tangent/=max(float(np.linalg.norm(tangent)),1e-8)
        bitangent=np.cross([dx,dy,dz],tangent)
        px,py,pz=x-cx,y-cy,z-cz
        qx=px*tangent[0]+py*tangent[1]+pz*tangent[2]
        qy=px*bitangent[0]+py*bitangent[1]+pz*bitangent[2]
        qz=px*dx+py*dy+pz*dz
        if a.gesture_version in ['20b','21','21b','22','22b']:
            # Resolved inlet shear: break the perfectly flat moving nozzle
            # front without filtering, deforming or recolouring emitted gas.
            qz=qz-.025*torch.sin(qx*22-t*13)*torch.cos(qy*25+t*11)
            qx=qx*(1+.10*torch.sin(qy*24+t*9))
            qy=qy*(1+.08*torch.sin(qx*27-t*7))
        source_now=torch.exp(-((qx/.27)**4+(qy/.22)**4+(qz/.085)**4))
        if a.gesture_version=='21b':
            # A rounded upstream ignition region avoids a hard disc-shaped
            # cut at the moving hand. Downstream combustion is unchanged.
            source_now=torch.exp(-((qx/.27)**4+(qy/.22)**4+(qz/.15)**2))
        source_now=(source_now>.01)*source_now
        vx_in,vy_in,vz_in=[torch.ones_like(x)*v for v in authored['incoming']]
        if a.gesture_version in ['20b','21','21b','22','22b']:
            shear_a=.40*torch.sin(qy*26+t*12)*torch.cos(qx*18-t*7)
            shear_b=.36*torch.cos(qx*25-t*11)*torch.sin(qy*19+t*8)
            vx_in+=shear_a*tangent[0]+shear_b*bitangent[0]
            vy_in+=shear_a*tangent[1]+shear_b*bitangent[1]
            vz_in+=shear_a*tangent[2]+shear_b*bitangent[2]
        inject=(source_now*inlet*dt*35).clamp(0,1)
    elif a.control=='orbit':
        # Move the actual fuel inlet. Previously the finished column was bent;
        # here every parcel reacts/advects from where the source really was.
        omega=2*math.pi/a.orbit_period;theta=omega*t
        cx=.72*math.sin(theta);cy=.10*math.sin(2*theta);cz=.34+.65*(1-math.cos(theta))
        source_now=torch.exp(-(((x-cx)/.27)**4+((y-cy)/.22)**4+((z-cz)/.085)**4))
        source_now=(source_now>.01)*source_now
        vx_in=torch.ones_like(x)*(.72*omega*math.cos(theta))
        vy_in=torch.ones_like(y)*(.20*omega*math.cos(2*theta))
        vz_in=torch.ones_like(z)*(1.15+.65*omega*math.sin(theta))
        inject=(source_now*inlet*dt*35).clamp(0,1)
    elif a.control=='directed':
        # Choreography acts on gas entering the solver and on native forces.
        # The emitted volume is never spatially bent, fitted or transformed.
        strength=a.control_strength
        # A rooted fuel jet feeds an extended circulating flame. Moving a tiny
        # injector around the domain produced disconnected ignition blobs.
        cx=strength*(.65+.10*math.sin(t*2));cz=.34+strength*.27
        source_now=torch.exp(-(((x-cx)/.31)**4+(y/.245)**4+((z-cz)/.115)**4))
        vx_in=torch.ones_like(x)*strength*(4.5+.55*math.sin(t*2.3))
        vy_in=torch.ones_like(y)*strength*.22*math.sin(t*2.7)
        vz_in=torch.ones_like(z)*(1.35+strength*.6*math.sin(t*1.8))
        inject=(source_now*inlet*dt*35).clamp(0,1)
    else:
        reveal=((t-.35)/2.25*12.-6.)
    ...
    v=state[0,:3]
    v[0].lerp_(vx_in if a.control in ['directed','orbit','choreographed'] else torch.ones_like(x)*(.12*math.sin(t*4.1)),inject)
    v[1].lerp_(vy_in if a.control in ['directed','orbit','choreographed'] else torch.ones_like(y)*(.11*math.cos(t*3.7)),inject)
    v[2].lerp_(vz_in*modulation if a.control in ['directed','orbit','choreographed'] else 1.35*modulation,inject)
...
def radiance():
    ...
    hot=((temp-.35)/1.65).clamp(0,1)
    sigma=(soot*3.6+reaction*.09).clamp(0,16)
    # Offline directional Beer shadowing. Baked light, not per-frame site lights.
    tau=torch.flip(torch.cumsum(torch.flip(soot,[0]),dim=0),[0])*h[2]*3.6
    key=torch.exp(-tau)
    colors=torch.stack([torch.ones_like(hot),.10+.85*hot.pow(.8),.004+.56*hot.pow(2)],dim=-1)
    emission=colors*(reaction.pow(.85)*10)[...,None]
    light=torch.tensor([.045,.037,.052],device=device)+key[...,None]*torch.tensor([1.8,1.55,1.45],device=device)
    rgb=emission+soot[...,None]*3.6*.55*light/(4*math.pi)
    return rgb,sigma

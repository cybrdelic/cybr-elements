...
ds.resolution_max=192;ds.cache_type='ALL';ds.cache_directory=str(ROOT/'cache-v2');ds.cache_frame_start=1;ds.cache_frame_end=144
ds.simulation_method='FLIP';ds.time_scale=.12;ds.use_mesh=True;ds.mesh_scale=2;ds.mesh_smoothen_pos=2;ds.mesh_smoothen_neg=2
...
bpy.ops.mesh.primitive_uv_sphere_add(segments=24,ring_count=16,radius=.095);nozzle=bpy.context.object;nozzle.name='Moving water source';nozzle.hide_render=True
fm=nozzle.modifiers.new('Liquid inflow','FLUID');fm.fluid_type='FLOW';fs=fm.flow_settings;fs.flow_type='LIQUID';fs.flow_behavior='INFLOW';fs.use_initial_velocity=True;fs.velocity_factor=0;fs.subframes=8
...
for frame in range(1,145):
 ...
 q=np.array([np.interp(min(12.8,pt+.025),times,points[:,0])-x,0,np.interp(min(12.8,pt+.025),times,points[:,1])-z]);q/=max(.0001,np.linalg.norm(q));fs.velocity_coord=tuple(q*.12+np.array([0,.10,.18]));fs.keyframe_insert('velocity_coord',frame=frame)
 fs.use_inflow=.25<=t<=3.6;fs.keyframe_insert('use_inflow',frame=frame)
ground=cube('Wet stone stage',(0,0,-1.42),(200,200,.2));ef=ground.modifiers.new('Liquid collision','FLUID');ef.fluid_type='EFFECTOR'
...
bpy.ops.object.camera_add();cam=bpy.context.object;s.camera=cam;cam.data.lens=36
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'motion-study.blend'))
(ROOT/'bake-report.json').write_text(json.dumps({'solver':'Mantaflow FLIP','resolution':192,'frames':144,'slowMotionTimeScale':.12,'seconds':elapsed,'baked':ds.has_cache_baked_data,'meshBaked':ds.has_cache_baked_mesh},indent=2))
